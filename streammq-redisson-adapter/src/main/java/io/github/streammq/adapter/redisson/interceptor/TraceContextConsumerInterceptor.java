 ...
 * <p>在消费前后处理追踪上下文：
 * <ul>
 *   <li>{@link #beforeConsume}: 从消息 userProperties 读取 traceId，放入 MDC</li>
 *   <li>{@link #afterConsume}: 从 MDC 移除 traceId，记录消费结果到 {@link TraceCollector}</li>
 * </ul>
 *
...
public class TraceContextConsumerInterceptor implements ConsumerInterceptor {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(TraceContextConsumerInterceptor.class);
    ...
    public void afterConsume(Message<?> message, Action action) {
        ...
        try {
            ...
            // 追踪上报失败不影响主流程
            LOG.warn("记录消费追踪失败: topic={}, messageId={}", message.getTopic(), message.getMessageId(), ex);
        }
    }
