 ...
 * 事务消息 Redis 联动集成测试。
 *
 * <p>实际操作本地 Redis（{@code localhost:6379}），覆盖 {@link TransactionScanner} 的
 * 半消息注册、显式 COMMIT/ROLLBACK、回查触发、{@link TransactionChecker} 回调等全流程。
 *
 * <p>测试策略：
 * <ul>
 *   <li>直接调用 {@link TransactionScanner#registerHalfMessage} 写入半消息</li>
 *   <li>调用 {@link TransactionScanner#markCommit} / {@link TransactionScanner#markRollback} 验证显式终结</li>
 *   <li>启动调度器并注册 {@link TransactionChecker},验证回查触发与状态流转</li>
 * </ul>
 *
...
class TransactionMessageIT extends AbstractRedisIT {
    ...
    void testHalfMessageRegistration() {
        ...
        try {
            ...
            StreamMessageId halfId = scanner.registerHalfMessage(txId, TX_GROUP, targetTopic, fields);
    ...
    void testCheckerCommitTransfersMessage() {
        ...
        try {
            String txId = UUID.randomUUID().toString();
            String targetTopic = "tx-checker-commit-" + UUID.randomUUID().toString().substring(0, 6);
            Map<String, String> fields = buildFields(targetTopic, "checker-commit-body");

            scanner.registerHalfMessage(txId, TX_GROUP, targetTopic, fields);

            // 注册回查器，返回 COMMIT_MESSAGE
            scanner.registerChecker(TX_GROUP, (TransactionChecker<Object>) (message, ctx) -> {
    ...
    void testCheckerRollbackDiscardsMessage() {
        ...
        try {
            String txId = UUID.randomUUID().toString();
            String targetTopic = "tx-checker-rollback-" + UUID.randomUUID().toString().substring(0, 6);
    ...
    void testCheckerUnknownReschedulesAndCounterIncrements() {
        ...
        try {
            String txId = UUID.randomUUID().toString();
            String targetTopic = "tx-checker-unknown-" + UUID.randomUUID().toString().substring(0, 6);
    ...
    void testNoCheckerForcesRollback() throws Exception {
        ...
        try {
            ...
            java.lang.reflect.Method m = TransactionScanner.class.getDeclaredMethod("scanTimeoutHalf", String.class);
    ...
    private Map<String, String> buildFields(String targetTopic, String body) {
        Message<String> message = MessageBuilder.<String>withTopic(targetTopic)
            .tag("tx-tag")
            .body(body)
            .build();
