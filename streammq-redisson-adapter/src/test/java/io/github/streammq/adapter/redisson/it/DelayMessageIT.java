 ...
 * 延时消息 Redis 联动集成测试。
 *
 * <p>实际操作本地 Redis（{@code localhost:6379}），覆盖 {@link DelayMessageScheduler} 与
 * {@link RedissonStreamProducer} 的延时消息全流程：发送、调度、转投、清理。
 *
...
class DelayMessageIT extends AbstractRedisIT {
    ...
    void testDelayMessageTransferredToTargetStream() {
        ...
        DelayMessageScheduler scheduler = new DelayMessageScheduler(
            redisson, namespace, SCAN_INTERVAL_MS, BATCH_SIZE);
    ...
    void testDifferentDelayLevelsDeliveredInOrder() {
        ...
        try {
            // 同时发送 1 秒级和 5 秒级延时消息
            Message<String> msg1 = MessageBuilder.<String>withTopic(topicLevel1)
                .tag("level1")
                .body("payload-l1")
                .delayLevel(DelayLevel.SECOND_1)
                .build();
            Message<String> msg5 = MessageBuilder.<String>withTopic(topicLevel5)
                .tag("level5")
                .body("payload-l5")
    ...
    private RedissonStreamProducer newProducer(String group) {
        return new RedissonStreamProducer(redisson, namespace, group, converter, 3000L, 0);
    }
}
