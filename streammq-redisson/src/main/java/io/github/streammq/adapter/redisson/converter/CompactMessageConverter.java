 ...
 *
 * <ul>
 *   <li>使用单字母/双字母字段名（{@code b} / {@code g} / {@code k} 等），降低每条 Entry 的存储体积
 *   <li>适用于海量消息、对存储成本敏感的场景
 *   <li>系统属性与用户属性<b>分别存储</b>为独立的 JSON 字段（{@code p} / {@code up}）
 *   <li>支持延迟字段（{@code dl} / {@code dm}）写入 Stream Entry
 *   <li>Topic 字段直接写入 Stream Entry（{@code t}），而非仅由 Stream Key 表示
 *   <li><b>不支持消息体压缩</b>（无 {@code compressed} 字段处理）
 * </ul>
 *
...
public class CompactMessageConverter extends AbstractMessageConverter {
     ...
     *
     * <p>支持 SDK 发送方（{@code bt} 非空 → Base64 解码 → 反序列化） 和跨平台发送方（{@code bt} 缺失 → 原始字符串处理）。
     * <b>不支持压缩解压。</b>
     *
