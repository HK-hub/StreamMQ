 ...
 * 消费者拦截器链默认实现（策略类）。
 *
 * <p>管理全局 {@link ConsumerInterceptor} 列表，并按 {@link ConsumerInterceptor#order()} 升序执行。 封装三阶段拦截逻辑：
 *
 * <ul>
 *   <li>{@link #applyBefore} - 消费前拦截，任一拦截器返回 false 则中止消费
 *   <li>{@link #applyAfter} - 消费后拦截，传入最终 {@link ConsumeAction}
 *   <li>{@link #notifyException} - 异常通知，传入 {@link InvokeTiming} 触发时机
 * </ul>
 *
...
public class DefaultConsumerInterceptorChain implements ConsumerInterceptorChain {
    ...
    private static final Logger LOG =
            LoggerFactory.getLogger(DefaultConsumerInterceptorChain.class);

    @Getter private final List<ConsumerInterceptor> interceptors = new CopyOnWriteArrayList<>();
        ...
        for (ConsumerInterceptor interceptor : interceptors) {
            try {
                if (!interceptor.beforeConsume(message, context)) {
                    LOG.debug(
                            "ConsumerInterceptor {} aborted consume: topic={}",
                            interceptor.name(),
                            message.getTopic());
                    return false;
                }
                ...
                LOG.warn(
                        "ConsumerInterceptor {} beforeConsume threw exception: {}",
                        interceptor.name(),
                        ex.getMessage(),
                        ex);
    ...
    public void applyAfter(Message<?> message, ConsumeAction action, ConsumeContext context) {
        for (ConsumerInterceptor interceptor : interceptors) {
            try {
                ...
                LOG.warn(
                        "ConsumerInterceptor {} afterConsume threw exception: {}",
                        interceptor.name(),
                        ex.getMessage(),
                        ex);
