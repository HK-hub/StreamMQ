...
public class RedissonStreamProducer implements StreamMessageProducer {
  ...
  public List<SendResult> syncSendBatch(List<? extends Message<?>> messages) {
    ...
    try {
      ...
      throw new StreamMQBrokerException("syncSendBatch failed for topic " + firstTopic, null, ex);
    }
    ...
    for (Message<?> message : messageList) {
      ...
      MessageId placeholder =
          MessageId.of(System.currentTimeMillis(), Math.abs(UUID.randomUUID().hashCode()));
      message.setMessageId(placeholder);
      results.add(
          new SendResult(
              placeholder, message.getTopic(), message.getTag(), message.getBornTimestamp()));
    }
    return results;
  }
