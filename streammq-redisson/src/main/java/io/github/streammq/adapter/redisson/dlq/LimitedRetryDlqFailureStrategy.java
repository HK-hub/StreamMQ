 ...
 * <p>DLQ 消费失败时：
 * <ul>
 *   <li>若 {@code dlqAttempts < maxDlqRetryAttempts} → 按退避延迟重试</li>
 *   <li>否则 → 丢弃</li>
 * </ul>
 *
 * <p>重试延迟计算：{@code min(baseDelay × multiplier^attempts, maxDelay)}
 * 当 multiplier=1.0 时为固定延迟。
 *
 * <p>适用场景：DLQ 消息可能因临时故障（如外部服务不可用）导致消费失败，
 * 允许有限次重试后放弃。
 *
...
public class LimitedRetryDlqFailureStrategy extends AbstractDlqFailureStrategy {
    ...
    protected DlqFailureDecision doDecide(Message<?> message, DlqFailureContext context) {
        ...
        if (attempts >= maxRetries) {
            log.warn("DLQ retry exhausted: attempts={}/{}, dropping message (topic={})",
                attempts, maxRetries, context.originalTopic());
            return DlqFailureDecision.drop();
        }

        Duration delay = computeDelay(attempts);
        log.info("DLQ retry scheduled: attempt={}/{}, delay={}ms (topic={})",
            attempts + 1, maxRetries, delay.toMillis(), context.originalTopic());
