...
public class DefaultStreamMQListenerContainer implements StreamMQListenerContainer {
  ...
  public DefaultStreamMQListenerContainer(
    ...
    this.inflightCapacity =
        inflightCapacity > 0 ? inflightCapacity : StreamMQConstants.DEFAULT_INFLIGHT_CAPACITY;
   ...
   *
   * <p>适用于需要完全自定义 ACK/重试/DLQ 路由的高级场景。此时 per-consumer 注解策略实例化关闭， 所有消费者共用传入的 {@code retryDlqHandler}。
   *
  ...
  public <T> void registerOrderlyConsumer(
    ...
    registrations.put(reg.key(), reg);
    LOG.info(
        "Registered StreamMQ Orderly Consumer: topic={}, group={}, shardCount={}, bodyType={}",
        annotation.topic(),
        annotation.consumerGroup(),
  ...
  private void resolvePerConsumerSpi(ListenerRegistration<?> reg) {
    ...
    // 2. per-consumer 重试策略
    RetryPolicy policy =
        SpiResolver.resolveOrInstantiate(reg.getRetryPolicy(), RetryPolicy.class, this.retryPolicy);
    ...
    LOG.debug(
        "Resolved per-consumer SPI: key={}, retryPolicy={}, converter={}, dlqFailureStrategy={},"
            + " filters={}",
        reg.key(),
        policy.name(),
  ...
  public void registerRetryTargets(RetryScheduler scheduler) {
    ...
    LOG.info(
        "Registered {} retry targets to RetryScheduler ({} DLQ listeners skipped)",
        count,
  ...
  private void consumeLoop(ListenerRegistration reg, boolean retryMode) {
    ...
    try {
      ...
      LOG.error(
          "Failed to create consumer for listener (topic={}, group={}, retryMode={}): {}, listener"
              + " will not consume",
          reg.getTopic(),
          reg.getGroup(),
          retryMode,
    ...
    LOG.info(
        "Consume loop started: topic={}, group={}, retryMode={}, listener={}",
        reg.getTopic(),
        reg.getGroup(),
        retryMode,
      ...
      while (state.get() == ContainerState.RUNNING) {
        ...
        try {
          List<Message<?>> messages =
              listener.pullBlock(
                  reg.getPullBatchSize(), Duration.ofMillis(reg.getPullBlockTimeoutMillis()));
  ...
  private void processMessage(
    ...
    try {
      ...
      try {
        if (reg.isDlqMode()) {
          LOG.debug(
              "Processing DLQ message: topic={}, group={}, messageId={}, consumerType={}",
              reg.getTopic(),
              reg.getGroup(),
              message.getMessageId(),
              reg.getConsumer().getClass().getSimpleName());
  ...
  private void processWithTimeout(
        ...
        consumeExecutor.submit(
            () -> {
              ...
              StreamMessageConcurrentlyConsumer consumer =
                  (StreamMessageConcurrentlyConsumer) reg.getConsumer();
              LOG.debug(
                  "processWithTimeout executing onMessage: topic={}, group={}, consumerClass={}",
                  reg.getTopic(),
                  reg.getGroup(),
                  consumer.getClass().getSimpleName());
              ConsumeAction action = consumer.onMessage(message, ctx);
    ...
    try {
      ...
      future.cancel(true);
      LOG.warn(
          "Consume timeout ({}ms) for message, cancelling and retrying: topic={}, group={},"
              + " messageId={}",
          reg.getConsumeTimeoutMillis(),
          reg.getTopic(),
          reg.getGroup(),
  ...
  private ConsumerFilter resolveConsumerFilter(Class<? extends ConsumerFilter> filterClass) {
    ...
    if (Objects.nonNull(resolver)) {
      ...
      LOG.debug(
          "Filter {} not resolved by filterResolver, trying reflection", filterClass.getName());
    }
