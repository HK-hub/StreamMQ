 ...
 *
 * <h3>设计意图</h3>
 * <p>三个 Converter 实现（{@link DefaultMessageConverter} / {@link CompactMessageConverter} /
 * {@link PassThroughMessageConverter}）在 {@code toStreamFields} 和 {@code fromStreamFields}
 * 中存在大量重复的结构化代码（null 检查、字段赋值、bornTs 解析等），唯一差异在于：
 * <ul>
 *   <li>字段名（长字段名 vs 短字段名 vs 直通）</li>
 *   <li>Body 序列化/反序列化方式（序列化器 vs toString）</li>
 *   <li>Properties 存储策略（合并 JSON vs 分离 JSON）</li>
 *   <li>扩展字段（延迟字段、topic 字段等）</li>
 * </ul>
 *
 * <h3>模板方法结构</h3>
 * <pre>
...
public abstract class AbstractMessageConverter implements MessageConverter {
     ...
     */
    protected String fieldBodyTypeName() { return null; }
     ...
     * @return 字段名，默认 null
     */
    protected String fieldRetryTimes() { return null; }
     ...
     * @return 字段名，默认 null
     */
    protected String fieldTxId() { return null; }
     ...
     * @return 字段名，默认 null
     */
    protected String fieldOriginTopic() { return null; }
     ...
     * @return 字段名，默认 null
     */
    protected String fieldDelayLevel() { return null; }
     ...
     * @return 字段名，默认 null
     */
    protected String fieldDelayTimeMillis() { return null; }
     ...
     */
    protected String fieldTopic() { return null; }
     ...
     *
     * <p>典型实现：序列化 body 为 byte[] → Base64 编码 → 写入字段。
     * 同时应写入 {@link #fieldBodyType()} 字段以支持消费端类型定位。
     *
     ...
     * <p>需处理两种来源：
     * <ol>
     *   <li>SDK 发送方：{@code bodyTypeField} 非空 → body 为 Base64 编码的序列化字节</li>
     *   <li>跨平台发送方：{@code bodyTypeField} 为空 → body 为原始字符串</li>
     * </ol>
     *
     ...
     * @param <T>        body 泛型类型
     */
    protected abstract <T> void decodeBody(Map<String, String> fields, Class<T> targetType, Message<T> message, String bodyStr);

    /**
     * 将消息的系统属性和用户属性编码写入 Stream Entry 字段。
     *
     * <p>典型实现：调用 {@link #writePropsJson(Map, String, Map, Map)} 合并写入单个 JSON 字段，
     * 或分别写入两个独立字段。
     *
     * @param message 消息载体
     * @param fields  输出 Map，直接修改
     ...
     * 编码扩展字段（钩子方法，默认空实现）。
     *
     * <p>用于写入模板方法未覆盖的自定义字段，如延迟级别、自定义延时毫秒等。
     * 在 {@link #toStreamFields(Message)} 末尾调用。
     *
     * @param message 消息载体
     * @param fields  输出 Map，直接修改
     */
     ...
     *
     * <p>用于读取模板方法未覆盖的自定义字段。
     * 在 {@link #fromStreamFields(Map, Class)} 末尾调用。
     *
     ...
     * <ol>
     *   <li>null 校验</li>
     *   <li>调用 {@link #encodeBody(Message, Map)} — 子类实现</li>
     *   <li>写入 tag / keys / shardingKey（通过 {@link #putField} 封装 null 安全）</li>
     *   <li>调用 {@link #encodeProperties(Message, Map)} — 子类实现</li>
     *   <li>写入 bornTs / bornHost / txId / retryTimes</li>
     *   <li>调用 {@link #encodeExtra(Message, Map)} — 钩子</li>
     * </ol>
     */
    @Override
     ...
     *   <li>null 校验</li>
     *   <li>新建空 Message</li>
     *   <li>读取 topic（若 Converter 支持）</li>
     *   <li>调用 {@link #decodeBody(Map, Class, Message, String)} — 子类实现</li>
     *   <li>读取 tag / keys / shardingKey / bornHost / txId（通过 {@link #getField} 封装 null 安全）</li>
     *   <li>调用 {@link #decodeProperties(Message, Map)} — 子类实现</li>
     *   <li>解析 bornTs（带容错）</li>
     *   <li>解析 retryTimes（若 Converter 支持，带容错）</li>
     *   <li>调用 {@link #decodeExtra(Message, Map)} — 钩子</li>
     * </ol>
     */
    @Override
     ...
     * @param <T>    body 泛型类型
     */
    protected static <T> void getField(Map<String, String> fields, String name, Consumer<String> setter) {
     ...
     *
     * <p>Default 和 PassThrough Converter 共用此方法。
     * 两个 Map 均可能为空，合并后若为空则不做任何写入。
     *
     * @param fields    目标 Map
     ...
     */
    protected void writePropsJson(Map<String, String> fields, String fieldName,
                                   Map<String, String> sysProps, Map<String, String> userProps) {
    ...
    protected <T> void readPropsJson(Map<String, String> fields, String fieldName,
        ...
        try {
            consumer.accept(propsMapper.readValue(json, new TypeReference<Map<String, String>>() {}));
        } catch (JsonProcessingException ex) {
