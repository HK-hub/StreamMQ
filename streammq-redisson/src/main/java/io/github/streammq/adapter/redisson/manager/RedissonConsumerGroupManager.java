...
public class RedissonConsumerGroupManager implements ConsumerGroupManager {
  ...
  public void register() {
    if (running.get()) {
      LOG.warn(
          "ConsumerGroupManager already registered: group={}, instanceId={}", group, instanceId);
      return;
    }
    // 1. 写入 instances Hash
        ...
        topic.addListener(
            ...
            (channel, msg) -> {
              if ("REBALANCE".equals(msg)) {
                LOG.info(
                    "Received REBALANCE notification: group={}, instanceId={}", group, instanceId);
              }
            });
  ...
  public void heartbeat() {
    ...
    try {
      ...
      if (heartbeatFailCount >= HEARTBEAT_FAIL_THRESHOLD) {
        LOG.error(
            "Heartbeat failed {} times, consumer may be considered dead: group={}, instanceId={}",
            heartbeatFailCount,
            group,
            instanceId);
      }
  ...
  public List<Integer> rebalance(int shardCount) {
    ...
    try {
      if (!sem.tryAcquire(5, TimeUnit.SECONDS)) {
        LOG.debug(
            "Semaphore acquire failed for Rebalance: group={}, another instance is processing",
            group);
      ...
      try {
        ...
        for (Map.Entry<Integer, String> entry : shardToConsumer.entrySet()) {
          assignment.computeIfAbsent(entry.getValue(), k -> new ArrayList<>()).add(entry.getKey());
        }
        // 写入 assignment Hash
        ...
        for (Map.Entry<String, List<Integer>> entry : assignment.entrySet()) {
          String shardsCsv =
              String.join(",", entry.getValue().stream().map(String::valueOf).toList());
