 ...
 * 基于 Token 的鉴权实现。
 *
 * <p>构造时注入期望的 token，{@link #authenticate(String, String, String)} 将
 * {@code password} 参数作为 token 凭证，验证其是否与期望 token 匹配。
 *
 * <p>使用 {@link MessageDigest#isEqual(byte[], byte[])} 进行时间安全比较，
 * 防止通过耗时差异推断 token 内容的时序攻击。
 *
 * <p>当前实现不区分 resource，仅校验 token 是否匹配。
 * 适用于使用单一共享 Token 的简单鉴权场景；如需细粒度权限控制，请自行扩展。
 *
