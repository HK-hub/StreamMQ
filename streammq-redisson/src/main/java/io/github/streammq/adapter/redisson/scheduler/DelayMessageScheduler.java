...
public class DelayMessageScheduler implements StreamMQScheduler {
    ...
    for (String msgId : expired) {
      ...
      try {
        ...
        try {
          ...
          LOG.error(
              "CRITICAL: Failed to re-add msgId={} to delay ZSet, message may be lost: {}",
              msgId,
              reAddEx.getMessage(),
              reAddEx);
        }
  ...
  void scanExpiredCustom() {
    ...
    for (String msgId : expired) {
      ...
      try {
        ...
        LOG.error(
            "Failed to transfer custom delay message msgId={}: {}", msgId, ex.getMessage(), ex);
        try {
          zset.add(System.currentTimeMillis(), msgId);
          LOG.warn("Re-added msgId={} to custom delay ZSet for retry", msgId);
        } catch (RuntimeException reAddEx) {
          LOG.error(
              "CRITICAL: Failed to re-add msgId={} to custom delay ZSet, message may be lost: {}",
              msgId,
              reAddEx.getMessage(),
              reAddEx);
        }
  ...
  public void cleanupOrphanedEntries() {
    ...
    for (DelayLevel level : DelayLevel.values()) {
      totalCleaned +=
          cleanupOrphanedInZSet(StreamMQKeys.delayZSet(namespace, level.name()), level.name());
    }
