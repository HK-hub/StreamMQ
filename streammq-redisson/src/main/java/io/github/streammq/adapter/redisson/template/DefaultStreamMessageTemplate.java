...
public class DefaultStreamMessageTemplate implements StreamMessageTemplate {
  ...
  public <T> CompletableFuture<SendResult> asyncSend(Message<T> message) {
    ...
    try {
      ...
      if (!applyInterceptorsBefore(message)) {
        return CompletableFuture.failedFuture(new StreamMQException("Aborted by interceptor"));
      }
      StreamMessageProducer producer = resolveProducer(message.getTopic());
      long sendStart = System.nanoTime();
      return producer
          ...
          .whenComplete(
              (result, ex) -> {
                if (Objects.isNull(ex)) {
                  applyInterceptorsAfter(message, result);
                  recordSendMetrics(
                      message.getTopic(), Objects.nonNull(result) && result.isSuccess(), sendStart);
                } else {
                  Exception e =
  ...
  public <T> SendResult executeInTransaction(Message<T> message, TransactionCallback<T> callback) {
    ...
    switch (state) {
      ...
      case ROLLBACK_MESSAGE:
        LOG.warn(
            "Transaction rolled back: txId={}, message may need manual cleanup", transactionId);
   ...
   *   <li>执行本地事务回调
   *   <li>根据本地事务结果：COMMIT → 转投到业务 Stream；ROLLBACK → 删除半消息； UNKNOW → 保留半消息，等待 TransactionScanner 回查
   * </ol>
   *
   * @param message 消息
  ...
  private <T> SendResult executeInTransactionWithScanner(
    ...
    try {
      halfId =
          scanner.registerHalfMessage(transactionId, transactionGroup, message.getTopic(), fields);
      LOG.debug(
      ...
      throw new TransactionException(
          "Half message register failed: " + ex.getMessage(), transactionId, transactionGroup, ex);
    }
    ...
    SendResult halfResult =
        new SendResult(msgId, message.getTopic(), message.getTag(), message.getBornTimestamp());
    ...
    switch (state) {
      case COMMIT_MESSAGE:
        try {
          ...
          LOG.error(
              "Failed to commit transaction: txId={}: {}", transactionId, ex.getMessage(), ex);
          throw new TransactionException(
              "Transaction commit failed: " + ex.getMessage(), transactionId, transactionGroup, ex);
        }
        return halfResult;
      case ROLLBACK_MESSAGE:
        try {
          ...
          LOG.warn(
              "Failed to rollback transaction: txId={}: {}", transactionId, ex.getMessage(), ex);
        }
        return new SendResult(
            msgId,
            message.getTopic(),
    ...
    for (ProducerInterceptor interceptor : interceptors) {
      try {
        if (!interceptor.beforeSend(message)) {
          LOG.debug(
              "Interceptor {} aborted send: topic={}", interceptor.name(), message.getTopic());
          return false;
        }
      } catch (RuntimeException ex) {
        LOG.warn(
