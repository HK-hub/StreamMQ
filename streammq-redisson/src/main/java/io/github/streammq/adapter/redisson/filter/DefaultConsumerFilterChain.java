 ...
 * 消费者过滤器链默认实现（策略类）。
 *
 * <p>管理全局 {@link ConsumerFilter} 列表，并按 {@link ConsumerFilter#order()} 升序执行。
 * 任一过滤器返回 false 则消息被跳过。
 *
...
public class DefaultConsumerFilterChain implements ConsumerFilterChain {
    ...
    @Getter
    private final List<ConsumerFilter> filters = new CopyOnWriteArrayList<>();
        ...
        for (ConsumerFilter filter : filters) {
            try {
                if (!filter.accept(message)) {
                    LOG.debug("ConsumerFilter {} rejected message: topic={}, tag={}",
