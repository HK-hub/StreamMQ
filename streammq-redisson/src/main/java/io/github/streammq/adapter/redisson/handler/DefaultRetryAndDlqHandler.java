...
public class DefaultRetryAndDlqHandler implements RetryAndDlqHandler {
  ...
  public void handleAction(
    ...
    if (reg.isDlqMode()) {
      LOG.info(
          "Routing to handleDlqFailureWithStrategy: topic={}, group={}, messageId={}, cause={}",
          reg.getTopic(),
          reg.getGroup(),
          messageId,
  ...
  private void handleDlqFailureWithStrategy(
    ...
    try {
      ...
      LOG.info(
          "Calling dlqFailureStrategy.decide: strategy={}, dlqRetryCount={}, dlqReason={}",
      ...
      switch (decision.type()) {
        case RETRY ->
            scheduleDlqRetry(
                message, reg, listener, messageId, fields, dlqRetryCount, decision.retryDelay());
        case SECONDARY_DLQ -> {
        ...
        default -> {
          LOG.warn(
              "DLQ message dropped (topic={}, group={}, messageId={}, dlqRetryCount={})",
              reg.getTopic(),
              reg.getGroup(),
              messageId,
              dlqRetryCount);
      ...
      LOG.error(
          "DLQ failure strategy error, falling back to drop (topic={}, group={}, messageId={}): {}",
          reg.getTopic(),
          reg.getGroup(),
          messageId,
          ex.getMessage(),
  ...
  private void scheduleDlqRetry(
    ...
    payload.put(
        RetryScheduler.FIELD_TARGET_TOPIC, StreamMQConstants.DLQ_RETRY_TARGET_TOPIC_SENTINEL);
    ...
    try {
      ...
      String dlq2Key =
          StreamMQKeys.secondaryDlqStream(
              reg.getNamespace(), reg.getGroup(), dlqConfig.getSecondaryDlqKeyPrefix());
  ...
  public void handleReconsumeLater(
    ...
    try {
      ...
      if (retryCount >= reg.getMaxReconsumeTimes()) {
        LOG.warn(
            "Retry count exceeded consumer maxReconsumeTimes, routing to DLQ "
