 ...
 * <h3>算法</h3>
 * <ul>
 *   <li>当 {@code shards >= consumers}：前 {@code shards % consumers} 个 Consumer 各分
 *       {@code shards/consumers + 1} 个，其余各分 {@code shards/consumers} 个</li>
 *   <li>当 {@code shards < consumers}：前 N 个 Consumer 各分 1 个，其余不分</li>
 * </ul>
 *
 * <h3>特点</h3>
 * <ul>
 *   <li><b>精确均衡</b>：各 Consumer 间分片数最多相差 1</li>
 *   <li><b>高迁移量</b>：Consumer 数量变化后，几乎所有分片可能被重新分配（无一致性保证）</li>
 *   <li>分片<b>交错</b>分配：同一 Consumer 获得的分片 ID 不一定连续</li>
 * </ul>
 *
 * <p>适用场景：分片数较少（&lt; 100），Consumer 实例稳定不频繁变动的场景。
 * 如需大规模分片 + 频繁弹性伸缩，请使用 {@link ConsistentHashRebalanceStrategy}。
 * 如需连续分片范围做批量处理，请使用 {@link RangeRebalanceStrategy}。
 *
