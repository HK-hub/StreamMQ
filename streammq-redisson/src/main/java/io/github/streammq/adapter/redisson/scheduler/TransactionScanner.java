...
public class TransactionScanner implements StreamMQScheduler {
  ...
  public void start() {
    ...
    LOG.info(
        "TransactionScanner started, checkIntervalMs={}, maxCheckTimes={}, batchSize={}, groups={}",
        checkIntervalMs,
        maxCheckTimes,
        batchSize,
   ...
   *
   * <p>通过 Lua 原子抢占事务执行权（PREPARE/UNKNOWN → COMMITTING）， 多实例并发时只有最先执行 Lua 的实例获得执行权，其余实例见到 COMMITTING
   * 后重新执行。 无需分布式锁。
   *
   * @param txId 事务 ID
   * @param txGroup 事务组名
  ...
  private void doMarkCommit(String txId, String txGroup) {
    ...
    if (STATE_COMMIT.equals(oldState) || STATE_ROLLBACK.equals(oldState)) {
      LOG.debug(
          "markCommit ignored, transaction already terminal: txId={}, state={}", txId, oldState);
      return;
    }
    ...
    LOG.info(
        "Transaction committed: txId={}, txGroup={}, targetTopic={}", txId, txGroup, targetTopic);
  }
  ...
  private void doMarkRollback(String txId, String txGroup) {
    ...
    if (STATE_COMMIT.equals(oldState) || STATE_ROLLBACK.equals(oldState)) {
      LOG.debug(
          "markRollback ignored, transaction already terminal: txId={}, state={}", txId, oldState);
      return;
    }
  ...
  void scanTimeoutHalf(String txGroup) {
    ...
    for (String txId : timeoutTxIds) {
      try {
        ...
        LOG.warn(
            "triggerCheck failed: txId={}, txGroup={}: {}", txId, txGroup, ex.getMessage(), ex);
      }
    }
  }
  ...
  void triggerCheck(String txId, String txGroup) {
    ...
    TransactionContext ctx =
        new TransactionContext(txId, txGroup, null, System.currentTimeMillis(), new HashMap<>());
    ...
    switch (state) {
      ...
      case UNKNOW -> {
        if (checkCount >= maxCheckTimes) {
          ...
          LOG.debug(
              "Transaction check UNKNOWN, rescheduled: txId={}, checkCount={}, nextCheckAt={}",
              txId,
              checkCount + 1,
              nextCheckAt);
        }
  ...
  private void publishHalfToBusiness(String txGroup, String halfIdStr, String targetTopic) {
    ...
    try {
      ...
      LOG.warn(
          "XDEL half message after publish failed: halfId={}: {}", halfIdStr, ex.getMessage(), ex);
    }
  }
  ...
  private Message<?> readHalfMessage(String txGroup, String halfIdStr, String targetTopic) {
    ...
    if (StringUtils.isNotEmpty(bodyTypeName)) {
      ...
      if (Objects.isNull(bodyType)) {
        try {
          bodyType =
              Class.forName(bodyTypeName, false, Thread.currentThread().getContextClassLoader());
