 ...
 * 去相关抖动重试策略（AWS 推荐）。
 *
 * <p>AWS 推荐的"去相关抖动"（Decorrelated Jitter）算法可避免同步重试导致的惊群效应。
 * 原始有状态算法为 {@code delay = min(cap, random(base, prev * 3))}，其中 prev 为上一次的延迟。
 *
 * <p>由于 {@link RetryPolicy#nextRetryDelay(int, Message)} 接口为无状态调用
 * （仅接收 reconsumeTimes，无法获取上一次延迟），本实现使用公式模拟有状态行为：
 * <pre>
