 ...
 *
 * <p>使用 JDK 内置 {@link GZIPOutputStream} / {@link GZIPInputStream}，无需额外依赖。
 * 压缩比较高，但速度不如 LZ4 / Snappy。适用于对压缩率敏感、对 CPU 开销不敏感的场景。
 *
...
public class GzipCompressionCodec implements CompressionCodec {
    ...
    public byte[] compress(byte[] data) {
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream(Math.max(data.length / 2, BUFFER_SIZE));
