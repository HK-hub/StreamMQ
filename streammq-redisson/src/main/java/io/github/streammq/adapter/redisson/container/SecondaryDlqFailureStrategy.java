 ...
 *   <li>否则 → 转投到二级死信队列（{@code streammq:{ns}:dlq2:{group}}）</li>
 * </ul>
 *
 * <p>适用于需要多级死信归档的场景：一级 DLQ = 正常重试耗尽时进入，
 * 二级 DLQ = 一级 DLQ 消费也失败时进入，可配合人工审核系统。
 *
...
public class SecondaryDlqFailureStrategy extends AbstractDlqFailureStrategy {
    ...
    protected DlqFailureDecision doDecide(Message<?> message, DlqFailureContext context) {
        ...
        if (attempts >= maxRetries) {
            log.warn("DLQ retry exhausted, routing to secondary DLQ: attempts={}/{}, topic={}",
        ...
        log.info("DLQ retry scheduled: attempt={}/{}, delay={}ms (topic={})",
            attempts + 1, maxRetries, delayMs, context.originalTopic());
