...
public class RedisStreamMQTraceService implements StreamMQTraceService {
  ...
  private TraceRecord parseRecord(Map<String, String> fields) {
    ...
    try {
      ...
      boolean success =
          Boolean.parseBoolean(fields.getOrDefault(RedisTraceCollector.FIELD_SUCCESS, "false"));
      long timestamp =
          Long.parseLong(fields.getOrDefault(RedisTraceCollector.FIELD_TIMESTAMP, "0"));
      long durationMillis =
          Long.parseLong(fields.getOrDefault(RedisTraceCollector.FIELD_DURATION_MILLIS, "0"));
