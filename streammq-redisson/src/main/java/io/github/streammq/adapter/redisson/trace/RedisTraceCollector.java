 ...
 *
 * <p>按日期分片存储（date 格式 yyyyMMdd），便于按天查询与过期清理。
 * 当 {@code streammq.trace.enabled=true} 且 {@code streammq.trace.storage=redis} 时启用。
 *
...
public class RedisTraceCollector implements TraceCollector {
    ...
    public void recordSend(SendTraceContext context) {
        ...
        try {
            ...
            fields.put(FIELD_MESSAGE_ID, context.messageId() != null ? context.messageId().getStreamEntryId() : "");
            ...
            fields.put(FIELD_ATTRIBUTES, serializeAttributes(context.attributes()));
            writeTrace(fields);
        } catch (Exception ex) {
            LOG.warn("Failed to record send trace: topic={}, messageId={}: {}",
                context.topic(), context.messageId(), ex.getMessage());
        }
    }
    ...
    public void recordConsume(ConsumeTraceContext context) {
        ...
        try {
            ...
            LOG.warn("Failed to record consume trace: topic={}, messageId={}: {}",
                context.topic(), context.messageId(), ex.getMessage());
        }
    }
