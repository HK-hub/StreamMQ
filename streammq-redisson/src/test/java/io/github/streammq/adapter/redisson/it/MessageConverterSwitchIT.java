...
/**
 * 消息转换器切换(DefaultMessageConverter vs 自定义 PassThroughMessageConverter) 端到端 Redis 联动集成测试。
 *
...
class MessageConverterSwitchIT extends AbstractRedisIT {
    ...
    /**
     * 自定义直通消息转换器:body 直接以字符串形式存入 Stream Entry, 不经过序列化器与 Base64 编码。仅支持 String body。
     *
     * <p>与 {@link DefaultMessageConverter} 的差异:
     *
     * <ul>
     *   <li>{@code body} 字段为原始字符串,而非 Base64(序列化字节)
     *   <li>不写入 {@code bodyType} 之外的序列化相关字段(保留 bodyType 以便消费端识别类型)
     * </ul>
     */
    static class PassThroughMessageConverter implements MessageConverter {
        ...
        public Map<String, String> toStreamFields(Message<?> message) {
            ...
            fields.put(
                    DefaultMessageConverter.FIELD_BORN_TS,
                    Long.toString(message.getBornTimestamp()));
    ...
    void passThroughMessageConverter_stringBody_roundTrip() {
        ...
        RedissonStreamProducer producer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-p", passThroughConverter, 3000L, 0, 0, 0);
        RedissonStreamListener consumer =
                new RedissonStreamListener(
                        redisson, namespace, topic, group, "c1", passThroughConverter);
        createConsumerGroup(topic, group);
    ...
    void bothConverters_differentBodyFormat_bothRestored() {
        ...
        RedissonStreamProducer defaultProducer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-dp", defaultConverter, 3000L, 0, 0, 0);
        RedissonStreamProducer passProducer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-pp", passThroughConverter, 3000L, 0, 0, 0);

        try {
            defaultProducer.syncSend(
                    MessageBuilder.<String>withTopic(topicDefault).body("same-text").build());
            passProducer.syncSend(
                    MessageBuilder.<String>withTopic(topicPass).body("same-text").build());
            ...
            String defaultBody =
                    defaultRange.values().iterator().next().get(DefaultMessageConverter.FIELD_BODY);
            String passBody =
                    passRange.values().iterator().next().get(DefaultMessageConverter.FIELD_BODY);
