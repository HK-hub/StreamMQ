 ...
 *
 * <p>验证 {@link JacksonJsonSerializer} 与 {@link JdkSerializer} 在生产-消费全链路下的 正确性:发送、写入 Stream
 * Entry、读取、还原 body。 同时验证两种序列化器产生的 Stream Entry {@code body} 字段格式不同但都能正确还原, 并覆盖自定义对象({@link
 * Person})的序列化/反序列化。
 */
...
class SerializerSwitchIT extends AbstractRedisIT {
    ...
    void jdkSerializer_stringBody_roundTrip() {
        ...
        MessageSerializer<Object> jdkSerializer =
                (MessageSerializer<Object>) (MessageSerializer<?>) new JdkSerializer<String>();
        MessageConverter jdkConverter = new DefaultMessageConverter(jdkSerializer);

        RedissonStreamProducer producer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-p", jdkConverter, 3000L, 0, 0, 0);
        RedissonStreamListener consumer =
    ...
    void bothSerializers_differentBodyFormat_bothRestored() {
        ...
        RedissonStreamProducer jacksonProducer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-jp", jacksonConverter, 3000L, 0, 0, 0);
        RedissonStreamProducer jdkProducer =
                new RedissonStreamProducer(
                        redisson, namespace, group + "-jdp", jdkConverter, 3000L, 0, 0, 0);

        try {
            jacksonProducer.syncSend(
                    MessageBuilder.<String>withTopic(topicJackson).body("same-body").build());
            jdkProducer.syncSend(
                    MessageBuilder.<String>withTopic(topicJdk).body("same-body").build());
            ...
            String jacksonBody =
                    jacksonRange.values().iterator().next().get(DefaultMessageConverter.FIELD_BODY);
            String jdkBody =
                    jdkRange.values().iterator().next().get(DefaultMessageConverter.FIELD_BODY);
            ...
            RedissonStreamListener jdkConsumer =
                    new RedissonStreamListener(
                            redisson, namespace, topicJdk, group + "-jc", "c1", jdkConverter);
            try {
    ...
    void jdkSerializer_personBody_roundTrip() {
        ...
        @SuppressWarnings("unchecked")
        MessageSerializer<Object> jdkSerializer =
                (MessageSerializer<Object>) (MessageSerializer<?>) new JdkSerializer<Person>();
        MessageConverter jdkConverter = new DefaultMessageConverter(jdkSerializer);
