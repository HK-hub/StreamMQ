 ...
 * <ul>
 *   <li>DLQ 消费者正常接收死信消息
 *   <li>DLQ 消费者消费成功后 ACK（消息从 DLQ PEL 移除）
 *   <li>DLQ 消费者使用 dlqMode = true 标识
 *   <li>DLQ 消费者消费失败后直接丢弃（不进入重试循环）
 * </ul>
 */
...
class DlqConsumerIT extends AbstractRedisIT {
                ...
                Proxy.newProxyInstance(
                        ...
                        (proxy, method, args) ->
                                switch (method.getName()) {
                                    ...
                                    case "enable" -> true;
                                    case "selectorType" ->
                                            io.github.streammq.core.enums.SelectorType.TAG;
    ...
    void dlqConsumer_receivesDeadLetterMessage() {
        ...
        StreamMessageConcurrentlyConsumer<String> dlqListener =
                (msg, ctx) -> {
                    ...
                    .untilAsserted(
                            () -> {
                                RStream<String, String> dlqStream =
                                        redisson.getStream(dlqStreamKey);
                                assertThat(
        ...
        try {
            RedissonStreamProducer producer =
                    new RedissonStreamProducer(
                            redisson, namespace, group + "-p", converter, 3000L, 0, 0, 0);
            producer.syncSend(
                    MessageBuilder.<String>withTopic(topic).body("default-group-body").build());
            producer.close();
    ...
    void dlqConsumer_consumeFailure_dropsMessage() {
        ...
        RedissonStreamListenerFactory consumerFactory =
                new RedissonStreamListenerFactory(redisson, converter);
        java.util.concurrent.atomic.AtomicReference<Message<?>> handledMessage =
                new java.util.concurrent.atomic.AtomicReference<>();
        java.util.concurrent.atomic.AtomicReference<Throwable> handledCause =
                new java.util.concurrent.atomic.AtomicReference<>();
        DlqFailureStrategy recordingStrategy =
                new DlqFailureStrategy() {
                    @Override
        ...
        java.util.concurrent.atomic.AtomicInteger dlqAttempts =
            ...
            await().atMost(5, TimeUnit.SECONDS)
                    .untilAsserted(
                            () -> {
                                ...
                                assertThat(handledCause.get().getMessage())
                                        .isEqualTo("DLQ consumer also fails");
                            });
