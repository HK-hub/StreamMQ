...
>
> 除下方单独标注的**直读精确键**外，所有键支持 Spring Boot 宽松绑定（`camelCase` / `kebab-case` / `snake_case` 均可），下表统一用 `kebab-case`。
...
| `trace.storage` | `none` | 存储方式（枚举 code，大小写不敏感；`redis` 启用 Redis Stream 存储，其他值禁用） |
...
| `startup-warn` | `true` | 启动时输出管理端点暴露面提醒（仅日志、零行为影响）。**直读精确键**（不走 `@ConfigurationProperties` 宽松绑定）：仅识别 `streammq.admin.startup-warn`，写成 `startupWarn` 会静默失效；也可用 `-Dstreammq.admin.startup-warn=false` 抑制 |
...
自动装配时对以下值做合法性校验（抛 `StreamMQClientException`，即 `StreamMQException` 的子类；**不是** `IllegalArgumentException`，`catch` 时请按 `StreamMQException` 捕获）：所有 `>0`/`>=0` 约束（见上表）、`group.instance-timeout-ms >= heartbeat-interval-ms`、`retry.pel-claim-min-idle-ms >= max(35000ms, 3 × group.heartbeat-interval-ms)`（低于该值仍处理中的消息可能被误判为孤儿重投）、`producer.retry-times <= 16`（`MAX_SYNC_RETRY_TIMES`）、`admin.trusted-proxies` 必须是合法 CIDR。
