 ...
 * <p>覆盖场景:
 * <ul>
 *   <li>syncSend 同步发送 → Listener 接收</li>
 *   <li>asyncSend 异步发送 → Listener 接收</li>
 *   <li>sendOneway 单向发送 → Listener 接收</li>
 *   <li>延时消息:发送延时消息 → DelayMessageScheduler 转投 → Listener 接收</li>
 * </ul>
 *
...
class SpringBootEndToEndIT {
    ...
    void cleanupRedis() {
        if (redissonClient != null && !redissonClient.isShutdown()) {
            redissonClient.getKeys().deleteByPattern(
                "streammq:" + properties.getNamespace() + ":*");
        }
    }
    ...
    void syncSend_listenerReceivesMessage() {
        ...
        Message<String> msg = MessageBuilder.<String>withTopic(E2E_TOPIC)
            .tag("sync-tag")
            .keys("sync-key")
            .body(body)
            .build();
    ...
    void delayMessage_schedulerTransfersAndListenerConsumes() {
        ...
        await().atMost(15, TimeUnit.SECONDS).until(() -> e2eStringListener.receivedBodies.contains(body));
