 ...
 * 在 Bean 初始化后扫描字段并将对应的 {@link StreamMessageTemplate} 实例注入。
 *
 ...
 *   <li>按注解 {@code group} 复用 {@link StreamMessageTemplate} 实例（同 group 同实例）</li>
 *   <li>若用户已注册同名 {@code StreamMqTemplate} Bean，优先复用</li>
 *   <li>否则按注解 + 全局配置创建 {@link DefaultStreamMessageTemplate}</li>
 * </ol>
 *
...
public class StreamMQProducerBeanPostProcessor implements BeanPostProcessor, ApplicationContextAware {
    ...
    private final Map<String, StreamMessageTemplate<?>> templatesByGroup = new ConcurrentHashMap<>();
     ...
     */
    public StreamMQProducerBeanPostProcessor(StreamMessageProducerFactory producerFactory,
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (!StreamMessageTemplate.class.isAssignableFrom(field.getType())) {
    ...
    private void injectTemplate(Object bean, Field field, StreamMQProducer annotation, String beanName) {
        ...
        StreamMessageTemplate<?> template = resolveTemplate(group, annotation);
        try {
    ...
    private StreamMessageTemplate<?> resolveTemplate(String group, StreamMQProducer annotation) {
        // 复用同 group 已创建的 Template
        StreamMessageTemplate<?> cached = templatesByGroup.get(group);
        if (cached != null) {
            return cached;
        }
        synchronized (this) {
            ...
            if (applicationContext.containsBean(beanName)) {
                StreamMessageTemplate<?> userBean = applicationContext.getBean(beanName, StreamMessageTemplate.class);
            ...
            DefaultStreamMessageTemplate<Object> template = new DefaultStreamMessageTemplate<>(
