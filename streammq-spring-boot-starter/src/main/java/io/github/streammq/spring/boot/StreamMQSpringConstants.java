...
public final class StreamMQSpringConstants {
    ...
    /**
     * 默认消息体序列化器实现类：{@code JacksonJsonSerializer}（严格类型、无 gadget RCE 面、Redis 中人类可读）。
     *
     * <p>与 {@link io.github.streammq.core.StreamMQConstants#DEFAULT_SERIALIZER} 指向同一实现， 此处以 {@code
     * Class} 形式提供，供自动装配在配置缺失/解析为空时回退使用。
     *
     * <p><b>安全默认：</b>0.1.2 起默认值由 Fury（宽松模式）回退为 Jackson。Fury 吞吐更高，但其宽松模式允许把 Redis 中的字节流反序列化为
     * classpath 上任意类——这个 RCE 面会通过本 SDK 传播给所有下游应用， 而"安全"不应依赖用户先读完 README 的警告段。 需要高吞吐时由用户显式
     * opt-in：{@code streammq.producer.serializer=...FurySerializer} 并建议同时开启 {@code
     * streammq.producer.fury-require-class-registration=true}。
     */
    ...
    /**
     * {@link io.github.streammq.adapter.redisson.serializer.FurySerializer} 的全限定名。
     *
     * <p>按<b>类名字符串</b>而非 {@code Class} 字面量引用：Fury 在 {@code streammq-redisson} 中是 optional
     * 依赖，starter 不应持有编译期引用，否则未引入 Fury 的应用在装配期就会 {@code NoClassDefFoundError}。
     */
    public static final String FURY_SERIALIZER_CLASS_NAME =
