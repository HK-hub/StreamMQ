...
public class StreamMQListenerRegistrar
   ...
   *
   * <p>支持 ${} 占位符与 #{} SpEL 表达式的属性：topic、consumerGroup、namespace、 selectorExpression、consumerName。
   *
   * @param original 原注解
   * @return 解析后的代理注解
   */
  ...
  /**
   * 创建 {@link StreamMQTransactionConsumer} 注解的动态代理，覆盖 transactionGroup/namespace 为解析后的值，其余方法委托给原注解。
   *
   * @param original 原注解
   * @return 解析后的代理注解
   */
  private StreamMQTransactionConsumer resolveStreamMQTransactionListener(
    ...
    LOG.info(
        "Resolved @StreamMQTransactionConsumer attributes: transactionGroup={} -> {}, namespace={}"
            + " -> {}",
  ...
  private void registerStreamMQListeners() {
    ...
    for (Map.Entry<String, Object> entry : beans.entrySet()) {
      ...
      StreamMQConsumer annotation =
          AnnotationUtils.findAnnotation(bean.getClass(), StreamMQConsumer.class);
      if (annotation == null) {
        continue;
      }
      ...
      if (isOrderly && bean instanceof StreamMessageOrderlyConsumer listener) {
      ...
      } else if (bean instanceof StreamMessageConcurrentlyConsumer listener) {
        ...
        LOG.warn(
            "Bean {} annotated with @StreamMQConsumer does not implement "
                + "StreamMessageConcurrentlyConsumer or StreamMessageOrderlyConsumer, ignored",
            beanName);
      }
    }
  }
  ...
  private void registerDlqListeners() {
    ...
    for (Map.Entry<String, Object> entry : beans.entrySet()) {
      ...
      StreamMQDlqConsumer annotation =
          AnnotationUtils.findAnnotation(bean.getClass(), StreamMQDlqConsumer.class);
      if (annotation == null) {
        continue;
      }
      ...
      if (!(bean instanceof DlqMessageConsumer)) {
        LOG.error(
            "Bean {} annotated with @StreamMQDlqConsumer must implement DlqMessageConsumer, startup"
                + " failed",
            beanName);
        throw new IllegalStateException(
            "Bean "
                + beanName
                + " annotated with @StreamMQDlqConsumer must implement DlqMessageConsumer");
      }
      DlqMessageConsumer listener = (DlqMessageConsumer) bean;
      listenerContainer.registerDlqConsumer(listener, annotation);
      LOG.info("Registered DlqConsumer: bean={}, group={}", beanName, annotation.consumerGroup());
    }
  }
  ...
  private void registerTransactionListeners() {
    ...
    if (beans.isEmpty()) {
      LOG.info(
          "No @StreamMQTransactionConsumer beans found, TransactionScanner will have no checkers");
      return;
    }
    for (Map.Entry<String, Object> entry : beans.entrySet()) {
      String beanName = entry.getKey();
      ...
      StreamMQTransactionConsumer annotation =
          AnnotationUtils.findAnnotation(bean.getClass(), StreamMQTransactionConsumer.class);
      if (annotation == null) {
        continue;
      }
      // 解析 ${} 占位符与 #{} SpEL 表达式
