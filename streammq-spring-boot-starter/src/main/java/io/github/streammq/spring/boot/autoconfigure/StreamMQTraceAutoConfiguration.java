 ...
 * <ul>
 *   <li>注册 {@link RedisTraceCollector} 作为 {@link TraceCollector}，将追踪数据写入 Redis Stream</li>
 *   <li>注册 {@link RedisStreamMQTraceService} 作为 {@link StreamMQTraceService}，提供追踪查询能力</li>
 *   <li>注册追踪上下文拦截器，自动收集发送/消费事件</li>
 * </ul>
 *
 * <p>本配置在 {@link StreamMQCoreAutoConfiguration} 之前加载，
 * 确保 {@code RedisTraceCollector} 优先于 Noop/Slf4j 收集器注册。
 *
...
public class StreamMQTraceAutoConfiguration {
    ...
    public TraceCollector redisTraceCollector(RedissonClient redisson, StreamMQProperties properties) {
        LOG.info("Using RedisTraceCollector (trace storage=redis, namespace={})", properties.getNamespace());
    ...
    @ConditionalOnProperty(prefix = "streammq.trace", name = "storage", havingValue = "redis")
    public StreamMQTraceService redisStreamMQTraceService(RedissonClient redisson,
                                                           StreamMQProperties properties) {
        LOG.info("Using RedisStreamMQTraceService (trace storage=redis, namespace={})", properties.getNamespace());
    ...
    public TraceContextProducerInterceptor streamMQTraceProducerInterceptor(TraceCollector traceCollector) {
    ...
    public TraceContextConsumerInterceptor streamMQTraceConsumerInterceptor(TraceCollector traceCollector) {
