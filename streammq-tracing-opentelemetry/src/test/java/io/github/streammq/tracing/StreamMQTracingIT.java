...
class StreamMQTracingIT {
  ...
  private ConsumeContext createMockContext(String group, String consumerName, int reconsumeTimes) {
  ...
  class OpenTelemetrySpanTests {
    ...
    void shouldEndSpanWithSuccess() {
      Span span =
          tracing.getTracer().spanBuilder("test-span").setSpanKind(SpanKind.INTERNAL).startSpan();
