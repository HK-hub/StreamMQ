 ...
 * <ul>
 *   <li>主题拓扑图构建：生产者/消费者节点识别、路由计算</li>
 *   <li>消息链路构建：单条消息的完整生命周期追踪</li>
 *   <li>时间范围查询：批量消息链路查询</li>
 * </ul>
 *
...
class StreamMQTracingMockTest {
    ...
    class TopologyBuilding {
        ...
        void shouldIdentifyProducerNodes() {
            ...
            TraceRecord sendRecord = new TraceRecord(
                "msg-001", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now, 10L, "trace-001", Map.of()
            );
    ...
    class MessageTraceBuilding {
        ...
        void shouldBuildCompleteTrace() {
            ...
            TraceRecord sendRecord = new TraceRecord(
                "msg-001", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now - 1000L, 10L, "trace-001", Map.of()
            );
            TraceRecord consumeRecord = new TraceRecord(
                "msg-001", TOPIC, CONSUMER_GROUP, TraceType.CONSUME,
                true, now, 5L, "trace-001", Map.of()
            );
        ...
        void shouldMarkFailedStatus() {
            long now = System.currentTimeMillis();
            TraceRecord sendRecord = new TraceRecord(
                "msg-002", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now - 1000L, 10L, "trace-002", Map.of()
            );
            TraceRecord failRecord = new TraceRecord(
                "msg-002", TOPIC, CONSUMER_GROUP, TraceType.CONSUME,
                false, now, 5L, "trace-002", Map.of("errorMessage", "消费失败")
            );
        ...
        void shouldMarkDlqStatus() {
            long now = System.currentTimeMillis();
            TraceRecord sendRecord = new TraceRecord(
                "msg-003", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now - 2000L, 10L, "trace-003", Map.of()
            );
            TraceRecord dlqRecord = new TraceRecord(
                "msg-003", TOPIC + "-dlq", CONSUMER_GROUP, TraceType.CONSUME,
                false, now, 5L, "trace-003", Map.of("dlqReason", "重试超限")
            );
    ...
    class TimeRangeQuery {
        ...
        void shouldGroupByMessageId() {
            ...
            TraceRecord record1 = new TraceRecord(
                "msg-001", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now - 2000L, 10L, "trace-001", Map.of()
            );
            TraceRecord record2 = new TraceRecord(
                "msg-002", TOPIC, PRODUCER_GROUP, TraceType.SEND,
                true, now - 1000L, 8L, "trace-002", Map.of()
            );
