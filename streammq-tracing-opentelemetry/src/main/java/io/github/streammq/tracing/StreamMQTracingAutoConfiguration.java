 ...
 * <p>触发条件：
 * <ul>
 *   <li>classpath 中存在 {@link StreamMessageTemplate}</li>
 *   <li>属性 {@code streammq.tracing.otel.enabled=true}</li>
 * </ul>
 *
 * <p>装配内容：
 * <ul>
 *   <li>{@link OpenTelemetry}：若上下文不存在则创建 no-op 默认实例（建议用户自行注入真实 SDK 实例以启用导出）</li>
 *   <li>{@link StreamMQTracing}：追踪核心门面</li>
 *   <li>{@link OpenTelemetryProducerInterceptor} / {@link OpenTelemetryConsumerInterceptor}：追踪拦截器</li>
 *   <li>{@link StreamMQTopologyService}：当存在 {@link StreamMQTraceService} 与 {@link StreamMQListenerContainer} 时装配</li>
 * </ul>
 *
...
public class StreamMQTracingAutoConfiguration {
     ...
     *
     * <p>当用户未提供自定义 {@link OpenTelemetry} Bean 时使用 no-op 实例，追踪操作将优雅降级为空操作。
     * 建议用户通过 OTel Spring Boot Starter 或 Agent 注入真实 SDK 实例以启用数据导出。
     *
    ...
    public OpenTelemetry streamMQOpenTelemetry(StreamMQTracingProperties properties) {
        log.info("StreamMQ OpenTelemetry 追踪已启用，未检测到自定义 OpenTelemetry Bean，使用 no-op 默认实例；"
            + "serviceName={}, otlpEndpoint={}", properties.getServiceName(), properties.getOtlpEndpoint());
     ...
     */
    @Bean
    public OpenTelemetryProducerInterceptor openTelemetryProducerInterceptor(StreamMQTracing tracing) {
     ...
     */
    @Bean
    public OpenTelemetryConsumerInterceptor openTelemetryConsumerInterceptor(StreamMQTracing tracing) {
    ...
    public StreamMQTopologyService streamMQTopologyService(StreamMQTraceService traceService,
