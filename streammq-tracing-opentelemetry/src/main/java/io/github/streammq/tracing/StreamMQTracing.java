 ...
 * <p>核心能力：
 * <ul>
 *   <li>生产者 Span 创建与上下文注入：{@link #injectProducerSpan(Message)} 创建名为
 *       {@value #SPAN_PRODUCER_SEND} 的 PRODUCER Span，并将 W3C {@code traceparent} 头
 *       注入消息系统属性</li>
 *   <li>消费者 Span 创建与上下文提取：{@link #startConsumerSpan(Message, ConsumeContext)}
 *       从消息属性提取 {@code traceparent}，创建名为 {@value #SPAN_CONSUMER_CONSUME} 的
 *       CONSUMER Span 并建立远程父级关系</li>
 *   <li>Span 结束与状态记录：{@link #endSpan(Span, boolean)} /
 *       {@link #endSpan(Span, boolean, String)}</li>
 * </ul>
 *
 * <p>上下文传播遵循 W3C TraceContext 标准（<a href="https://www.w3.org/TR/trace-context/">W3C TraceContext</a>），
 * 通过消息系统属性 {@code traceparent} / {@code tracestate} 在生产者与消费者之间串联链路。
 * 当 OpenTelemetry 为 no-op 实现时（如未配置 SDK），Span 上下文无效，注入将自动跳过，实现优雅降级。
 *
 * <p>Span 命名约定：
 * <ul>
 *   <li>生产者发送：{@value #SPAN_PRODUCER_SEND}</li>
 *   <li>消费者消费：{@value #SPAN_CONSUMER_CONSUME}</li>
 *   <li>重试调度：{@value #SPAN_SCHEDULER_RETRY}</li>
 *   <li>延时调度：{@value #SPAN_SCHEDULER_DELAY}</li>
 *   <li>死信路由：{@value #SPAN_DLQ_ROUTE}</li>
 * </ul>
 *
...
public class StreamMQTracing {
    ...
    private final Map<Span, Long> spanStartNanos = Collections.synchronizedMap(new IdentityHashMap<>());
     ...
     *
     * <p>在 {@code ProducerInterceptor.beforeSend} 中调用。创建 PRODUCER 类型的 Span
     * （名称 {@value #SPAN_PRODUCER_SEND}），设置 Topic / Tag / 消息 ID 等属性，
     * 并将 W3C {@code traceparent} / {@code tracestate} 写入消息系统属性。
     *
     * <p>Span 存储在 ThreadLocal 中，需在发送完成后通过
     * {@link #getCurrentProducerSpan()} 取出并调用 {@link #endSpan(Span, boolean)} 结束。
     *
     * @param message 待发送消息
     */
    public void injectProducerSpan(Message<?> message) {
        ...
        try {
            Span span = tracer.spanBuilder(SPAN_PRODUCER_SEND)
     ...
     *
     * <p>在 {@code ProducerInterceptor.afterSend} / {@code onException} 中取出后，
     * 调用 {@link #endSpan(Span, boolean)} 结束，并调用 {@link #clearCurrentProducerSpan()} 清理。
     *
     ...
     *
     * <p>在 {@code ConsumerInterceptor.beforeConsume} 中调用。解析消息属性中的
     * W3C {@code traceparent}，创建 CONSUMER 类型的 Span（名称 {@value #SPAN_CONSUMER_CONSUME}），
     * 以提取的远程上下文作为父级，设置 Topic / 消费者组 / 重试次数等属性。
     *
     * @param message 待消费消息
     * @param context 消费上下文
    ...
    public Span startConsumerSpan(Message<?> message, ConsumeContext context) {
        ...
        try {
            ...
            Span span = tracer.spanBuilder(SPAN_CONSUMER_CONSUME)
    ...
    public void endSpan(Span span, boolean success, String errorMessage) {
        ...
        try {
            ...
            if (success) {
                ...
                span.setStatus(StatusCode.ERROR, StringUtils.isNotEmpty(errorMessage) ? errorMessage : "执行失败");
            }
            span.end();
        } catch (Exception ex) {
    ...
    private static SpanContext parseTraceparent(String traceparent, String tracestate) {
        ...
        TraceState state = StringUtils.isEmpty(tracestate) ? TraceState.getDefault() : parseTraceState(tracestate);
     ...
     *
     * <p>OpenTelemetry 1.40.0 的 {@link TraceState} 接口未直接提供序列化方法，
     * 此处通过 {@link TraceState#asMap()} 获取键值对后手动拼接。
     *
     ...
     *
     * <p>OpenTelemetry 1.40.0 的 {@link TraceState} 接口未提供静态 {@code parse} 方法，
     * 此处通过 {@link TraceStateBuilder} 手动构建。格式不合法的条目将被跳过。
     *
