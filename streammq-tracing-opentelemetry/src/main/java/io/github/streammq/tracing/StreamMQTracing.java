...
public class StreamMQTracing {
  ...
  public void injectProducerSpan(Message<?> message) {
    ...
    try {
      Span span = tracer.spanBuilder(SPAN_PRODUCER_SEND).setSpanKind(SpanKind.PRODUCER).startSpan();
      recordStart(span);
   ...
   *
   * <p>在 {@code ProducerInterceptor.afterSend} / {@code onException} 中取出后， 调用 {@link #endSpan(Span,
   * boolean)} 结束，并调用 {@link #clearCurrentProducerSpan()} 清理。
   *
   ...
   *
   * <p>在 {@code ConsumerInterceptor.beforeConsume} 中调用。解析消息属性中的 W3C {@code traceparent}，创建 CONSUMER
   * 类型的 Span（名称 {@value #SPAN_CONSUMER_CONSUME}）， 以提取的远程上下文作为父级，设置 Topic / 消费者组 / 重试次数等属性。
   *
   * @param message 待消费消息
   * @param context 消费上下文
  ...
  public Span startConsumerSpan(Message<?> message, ConsumeContext context) {
    ...
    try {
      ...
      Span span =
          tracer
              .spanBuilder(SPAN_CONSUMER_CONSUME)
  ...
  public void endSpan(Span span, boolean success, String errorMessage) {
    ...
    try {
      ...
      if (success) {
        ...
        span.setStatus(
            StatusCode.ERROR, StringUtils.isNotEmpty(errorMessage) ? errorMessage : "执行失败");
      }
      span.end();
    } catch (Exception ex) {
   ...
   * 会将系统属性与用户属性合并存储，反序列化后统一写入 {@code userProperties}。 若注入到系统属性，消费端将无法从 {@code getProperties()} 读回。
   *
   * @param span 生产者 Span
   * @param message 消息载体
   */
  ...
  private static SpanContext parseTraceparent(String traceparent, String tracestate) {
    ...
    TraceState state =
        StringUtils.isEmpty(tracestate) ? TraceState.getDefault() : parseTraceState(tracestate);
