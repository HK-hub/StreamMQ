...
public class GracefulShutdownHandler implements DisposableBean {
  ...
  public GracefulShutdownHandler(
      ObjectProvider<StreamMQListenerContainer> containerProvider, CloudK8sProperties properties) {
   ...
   * 等待处理中的消息完成。
   *
   * <p>pause() 仅停止拉取新消息，不改变容器的 running 状态，因此不能依赖 {@code isRunning()} 判断 in-flight 是否完成。此处采用有界短等待策略：
   * 等待固定宽限期（最多 1 秒），让 in-flight 消息自然完成，然后由 {@link #stopContainer} 统一关闭容器。
   *
