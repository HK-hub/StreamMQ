 ...
 * <ul>
 *   <li>{@code GET /streammq/health/liveness} - 返回存活状态，含 {@code status}、{@code backend} 字段</li>
 *   <li>{@code GET /streammq/health/readiness} - 返回就绪状态，含 {@code ready}、{@code consumerCount} 字段</li>
 * </ul>
 *
 * <p>当 {@link StreamMQListenerContainer} 不存在时，就绪探针返回降级状态
 * （{@code ready=false}、{@code consumerCount=0}），存活探针仍返回 UP。
 *
...
public class StreamMQHealthController {
     ...
     * 存活探针端点。
     *
     * <p>K8s livenessProbe 调用，返回 Pod 是否存活。存活状态恒为 UP，
     * 仅在后端不可达等极端场景下由上层网关降级。
     *
     ...
     * 就绪探针端点。
     *
     * <p>K8s readinessProbe 调用，返回 Pod 是否准备好接收流量。当监听器容器
     * 不存在或未运行时返回 {@code ready=false}，否则返回 {@code ready=true}
     * 与当前消费者数量。
     *
