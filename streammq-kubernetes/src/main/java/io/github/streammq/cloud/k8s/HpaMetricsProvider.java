 ...
 *
 * <p>为 Kubernetes 水平 Pod 自动扩缩容提供消费延迟与消费速率指标。
 * 默认实现使用内存数据结构（{@link ConcurrentHashMap}）存储指标值，
 * 可被子类覆盖以接入真实指标源（如 {@code streammq-redisson} 的 StreamMQMetrics）。
 *
 * <p>指标维度：
 * <ul>
 *   <li>消费延迟（lag）：未消费的积压消息数，用于触发扩容</li>
 *   <li>消费速率（rate）：每秒处理消息数，用于评估扩缩容效果</li>
 * </ul>
 *
 * <p>外部组件可通过 {@link #recordLag} 与 {@link #recordConsumeRate} 写入指标值，
 * K8s 自定义指标适配器通过 getter 方法读取并暴露给 HPA 控制器。
 *
