 ...
 *
 * <p>在 K8s 环境中，配置变更（如 ConfigMap 滚动更新）可通过实现本接口将新配置 推送到运行中的 {@link
 * io.github.streammq.core.listener.StreamMQListenerContainer}， 避免重启 Pod 即可生效。
 *
 * <p>限制说明：
 *
 * <ul>
 *   <li>不支持运行时变更 backend type（如 redis 切换到 rocketmq），此类变更需重启 Pod
 *   <li>刷新重试间隔时，需保证 {@code retryIntervals} 长度与 {@code maxReconsumeTimes} 一致
 *   <li>消费线程数刷新仅影响后续创建的消费线程，不会中断正在执行的任务
 * </ul>
 *
