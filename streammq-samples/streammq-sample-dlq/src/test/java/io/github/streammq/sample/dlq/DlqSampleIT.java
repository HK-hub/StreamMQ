...
class DlqSampleIT {
  ...
  private void cleanStreams() {
    try {
      ...
      String retryKey =
          "streammq:" + NAMESPACE + ":retry:msg:" + TOPIC + ":" + TEST_FAIL_CONSUMER_GROUP;
  ...
  void normalMessageDelivery() {
    ...
    System.out.println(
        "=== TestCollector.receivedMessages before send: " + testCollector.receivedMessages.size());
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              ...
              assertThat(received).isNotNull();
              assertThat(received.getBody()).isEqualTo(content);
              assertThat(received.getUserProperties()).containsEntry("source", "dlq-sample");
            });
  }
