 ...
 * 由独立的测试消费者组接收并验证，覆盖生产→存储→消费→死信全链路。
 *
 * <p>测试场景：
 * <ul>
 *   <li>{@code normalMessageDelivery} 正常消息投递 → 测试消费者接收并验证消息内容</li>
 *   <li>{@code failedMessageTriggersDlq} 消息消费失败超过重试次数 → 进入死信队列</li>
 *   <li>{@code dlqConsumerReceivesDeadLetter} 死信消费者从 DLQ Stream 接收到死信消息</li>
 * </ul>
 *
...
@Import({DlqSampleIT.TestMessageCollector.class, DlqSampleIT.TestFailConsumer.class,
        DlqSampleIT.TestDlqConsumer.class, DlqSampleIT.TestConfig.class})
...
class DlqSampleIT {
    ...
    private void cleanStreams() {
        try {
            ...
            String retryKey = "streammq:" + NAMESPACE + ":retry:msg:" + TOPIC + ":" + TEST_FAIL_CONSUMER_GROUP;
            String dlqKey = "streammq:" + NAMESPACE + ":dlq:" + TEST_FAIL_CONSUMER_GROUP;
            System.out.println("=== Cleaning streams: " + topicKey + ", " + retryKey + ", " + dlqKey);
    ...
    void normalMessageDelivery() {
        ...
        System.out.println("=== TestCollector.receivedMessages before send: " + testCollector.receivedMessages.size());
        ...
        await().atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    System.out.println("=== TestCollector.receivedMessages in await: " + testCollector.receivedMessages.size());
     ...
     *
     * <p>使用 {@link TestFailConsumer}（consumerGroup = test-fail-group）始终抛出异常，
     * 触发 DLQ 机制。通过 {@link TestDlqConsumer} 验证死信消息被接收。
     */
    @Test
    ...
    void failedMessageTriggersDlq() {
        ...
        await().atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    ...
                    Message<String> received = testDlqConsumer.receivedDlqMessages.stream()
    ...
    /**
     * 测试专用配置：覆盖全局 {@link RetryPolicy} 使用短间隔重试策略，
     * 使 DLQ 测试在秒级完成而非分钟级。
     */
    @Configuration
    ...
    static class TestMessageCollector implements StreamMessageConcurrentlyConsumer<String> {
        ...
        public ConsumeAction onMessage(Message<String> message, ConsumeContext context) {
            System.out.println("=== TestMessageCollector.onMessage called: keys=" + message.getKeys()
                + ", body=" + message.getBody() + ", instance=" + this.hashCode());
            receivedMessages.add(message);
            System.out.println("=== TestMessageCollector.receivedMessages size=" + receivedMessages.size());
     ...
     *
     * <p>使用独立的测试消费者组（{@value #TEST_FAIL_CONSUMER_GROUP}），
     * 确保不会与生产消费者组冲突。始终抛出 {@link RuntimeException}，
     * 模拟消费失败场景，验证 DLQ 机制。
     */
    @StreamMQConsumer(topic = "order-topic", consumerGroup = TEST_FAIL_CONSUMER_GROUP, maxReconsumeTimes = 3)
    static class TestFailConsumer implements StreamMessageConcurrentlyConsumer<String> {

        @Override
        public ConsumeAction onMessage(Message<String> message, ConsumeContext context) {
            throw new RuntimeException("Test intentional failure for DLQ: orderId=" + message.getKeys());
        }
    }
     ...
     * 测试专用死信消费者。
     *
     * <p>通过 {@link StreamMQDlqConsumer} 注解注册为 DLQ 消费者，
     * 监听 {@value #TEST_FAIL_CONSUMER_GROUP} 消费者组的死信队列，
     * 收集死信消息供测试验证。
     */
    ...
    static class TestDlqConsumer extends AbstractDlqMessageConsumer<String> {
        ...
        final ConcurrentLinkedQueue<Message<String>> receivedDlqMessages = new ConcurrentLinkedQueue<>();
