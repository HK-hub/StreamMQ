 ...
 * <p>演示死信队列消费：
 * <ul>
 *   <li>使用 {@link StreamMQDlqConsumer} 注解注册死信消费者</li>
 *   <li>继承 {@link AbstractDlqMessageConsumer} 实现死信消息处理</li>
 *   <li>配置 DLQ 失败策略（默认 LogAndDrop）</li>
 * </ul>
 *
 * <p>死信消息来源：当 {@code order-consumer-group} 的消息消费失败超过 maxReconsumeTimes 后，
 * 消息会被转移到死信队列 {@code streammq:{ns}:dlq:order-consumer-group}。
 *
...
public class OrderDlqConsumer extends AbstractDlqMessageConsumer<String> {
    ...
    public void onDlqMessage(Message<String> message, ConsumeContext context) throws Exception {
        log.info("Received DLQ message: topic={}, keys={}, body={}, reconsumeTimes={}, consumerGroup={}",
                message.getTopic(), message.getKeys(), message.getBody(),
                context.reconsumeTimes(), context.consumerGroup());

        try {
            processDlqMessage(message);
            log.info("DLQ message processed successfully: keys={}", message.getKeys());
        } catch (Exception e) {
            log.error("Failed to process DLQ message: keys={}, error={}",
