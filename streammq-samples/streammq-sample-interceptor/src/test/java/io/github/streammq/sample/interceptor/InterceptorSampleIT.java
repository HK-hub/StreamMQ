 ...
 * 由 {@link TestMessageCollector} 接收并验证拦截器注入的 traceId/spanId，
 * 覆盖生产者拦截器 → 存储 → 消费者拦截器全链路。
 *
 * <p>测试场景：
 * <ul>
 *   <li>{@code sendMessage_traceIdInjectedByInterceptor} 发送消息 → 验证 traceId 被 TraceProducerInterceptor 注入并被消费者接收</li>
 *   <li>{@code sendMessage_spanIdInjectedByInterceptor} 发送消息 → 验证 spanId 被正确注入</li>
 *   <li>{@code sendMultipleMessages_allReceived} 批量发送 → 验证多条消息全部被消费者接收</li>
 * </ul>
 *
