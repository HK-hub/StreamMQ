 ...
 * 顺序消息示例集成测试。
 *
 * <p>启动完整 Spring Boot 上下文，通过 {@link OrderlyMessageProducer} 发送消息，
 * 由测试专用的 {@link TestOrderlyMessageCollector} 按顺序接收，验证同一 shardingKey
 * 的消息严格按发送顺序消费。
 *
 * <p>测试场景：
 * <ul>
 *   <li>{@code sendOrderlyMessage_verifiedInOrder} 同步发送同 shardingKey 消息 → 验证顺序</li>
 *   <li>{@code sendOrderStatusFlow_allStatusesArriveInOrder} 订单状态流转 → 验证完整生命周期顺序</li>
 *   <li>{@code sendBatchOrderlyMessages_allMessagesArriveInOrder} 批量发送 → 验证所有消息按序到达</li>
 *   <li>{@code multipleShardingKeys_messagesPerShardInOrder} 多 shardingKey 并行 → 验证每个分片内有序</li>
 * </ul>
 *
...
class OrderlySampleIT {
    ...
    @Autowired
    private OrderlyMessageProducer producer;

    @Autowired
    private TestOrderlyMessageCollector testCollector;
    ...
    void sendOrderlyMessage_verifiedInOrder() {
        ...
        await().atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    List<Message<String>> messages = new ArrayList<>(testCollector.receivedMessages);
                    assertThat(messages).hasSize(3);
                    assertThat(messages.get(0).getKeys()).isEqualTo(orderId);
                    assertThat(messages.get(0).getUserProperties().get("sequence")).isEqualTo("1");
                    assertThat(messages.get(1).getKeys()).isEqualTo(orderId);
                    assertThat(messages.get(1).getUserProperties().get("sequence")).isEqualTo("2");
                    assertThat(messages.get(2).getKeys()).isEqualTo(orderId);
                    assertThat(messages.get(2).getUserProperties().get("sequence")).isEqualTo("3");
                });
    }
    ...
    void sendOrderStatusFlow_allStatusesArriveInOrder() {
        ...
        await().atMost(15, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    ...
                    assertThat(messages.get(0).getBody()).isEqualTo("{\"status\":\"created\"}");
                    assertThat(messages.get(1).getBody()).isEqualTo("{\"status\":\"paid\"}");
                    assertThat(messages.get(2).getBody()).isEqualTo("{\"status\":\"shipped\"}");
                    assertThat(messages.get(3).getBody()).isEqualTo("{\"status\":\"completed\"}");
                });
    }
    ...
    void multipleShardingKeys_messagesPerShardInOrder() {
        ...
        await().atMost(20, TimeUnit.SECONDS)
                .untilAsserted(() -> {
                    ...
                    List<Message<String>> order1Messages = messages.stream()
                            .filter(m -> orderId1.equals(m.getKeys())).toList();
                    List<Message<String>> order2Messages = messages.stream()
                            .filter(m -> orderId2.equals(m.getKeys())).toList();
                    List<Message<String>> order3Messages = messages.stream()
                            .filter(m -> orderId3.equals(m.getKeys())).toList();

                    assertThat(order1Messages).hasSize(3);
                    assertThat(order2Messages).hasSize(3);
                    assertThat(order3Messages).hasSize(3);

                    assertThat(order1Messages.get(0).getUserProperties().get("sequence")).isEqualTo("1");
                    assertThat(order1Messages.get(1).getUserProperties().get("sequence")).isEqualTo("2");
                    assertThat(order1Messages.get(2).getUserProperties().get("sequence")).isEqualTo("3");

                    assertThat(order2Messages.get(0).getUserProperties().get("sequence")).isEqualTo("1");
                    assertThat(order2Messages.get(1).getUserProperties().get("sequence")).isEqualTo("2");
                    assertThat(order2Messages.get(2).getUserProperties().get("sequence")).isEqualTo("3");

                    assertThat(order3Messages.get(0).getUserProperties().get("sequence")).isEqualTo("1");
                    assertThat(order3Messages.get(1).getUserProperties().get("sequence")).isEqualTo("2");
                    assertThat(order3Messages.get(2).getUserProperties().get("sequence")).isEqualTo("3");
                });
    }
