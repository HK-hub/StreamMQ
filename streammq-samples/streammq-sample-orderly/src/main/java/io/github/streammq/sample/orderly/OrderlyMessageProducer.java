 ...
 * 顺序消息生产者示例。
 *
 * <p>演示顺序消息发送：通过设置 {@code shardingKey} 确保同一业务实体的消息 在消费者端按发送顺序消费。
 *
...
public class OrderlyMessageProducer {
   ...
   * 发送订单顺序消息。
   *
   * <p>通过 {@code shardingKey} 指定分片键，确保同一订单的消息 在同一分片内按顺序消费。
   *
  ...
  public SendResult sendOrderlyMessage(String orderId, String content, int sequence) {
    log.info(
        "Sending orderly message: orderId={}, sequence={}, content={}", orderId, sequence, content);
    ...
    log.info(
        "Orderly message sent: orderId={}, sequence={}, msgId={}",
        orderId,
        sequence,
        result.getMessageId());
    return result;
  ...
  public void sendBatchOrderlyMessages(String orderId, int count) {
    for (int i = 1; i <= count; i++) {
      String content =
          String.format("{\"orderId\":\"%s\",\"sequence\":%d,\"data\":\"item-%d\"}", orderId, i, i);
