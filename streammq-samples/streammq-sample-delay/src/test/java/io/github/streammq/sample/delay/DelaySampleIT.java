 ...
 * <p>测试场景覆盖：
 * <ul>
 *   <li>{@link #sendCustomDelayMessage()} — 发送自定义延时消息（5 秒）并验证消费</li>
 *   <li>{@link #sendFixedDelayMessage()} — 发送固定延时消息（{@link DelayLevel#SECOND_10}）并验证消费</li>
 * </ul>
 *
 * <p>本测试使用独立的测试消费者组（{@code delay-order-consumer-group-it}），
 * 通过 {@link DelayMessageTestConsumer} 收集消息，配合 Awaitility 进行超时等待。
 *
...
class DelaySampleIT {
     ...
     * <p>验证要点：
     * <ul>
     *   <li>消息发送成功，返回非空 MessageId</li>
     *   <li>消息在超时时间内（15 秒）被测试消费者接收</li>
     *   <li>接收到的消息包含正确的 orderId、body 和 tag（{@code custom-delay}）</li>
     * </ul>
     */
    @Test
    ...
    void sendCustomDelayMessage() throws InterruptedException {
        ...
        assertThat(messages)
                .as("应接收到 1 条延时消息")
                .hasSize(1);
        assertThat(messages.get(0).getKeys())
                .as("消息 keys 应匹配 orderId")
                .isEqualTo(orderId);
        assertThat(messages.get(0).getBody())
                .as("消息 body 应匹配发送内容")
                .isEqualTo(content);
        assertThat(messages.get(0).getTag())
                .as("消息 tag 应为 custom-delay")
                .isEqualTo("custom-delay");
    }

    /**
     * 发送固定延时消息（{@link DelayLevel#SECOND_10}，10 秒延时时长），
     * 验证消息在延时期满后被正确消费。
     *
     * <p>验证要点：
     * <ul>
     *   <li>消息发送成功，返回非空 MessageId</li>
     *   <li>消息在超时时间内（20 秒）被测试消费者接收</li>
     *   <li>接收到的消息包含正确的 orderId、body 和 tag（{@code delay}）</li>
     * </ul>
     */
    @Test
