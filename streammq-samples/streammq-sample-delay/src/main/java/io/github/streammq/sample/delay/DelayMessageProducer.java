 ...
 * <p>演示两种延时消息发送方式：
 * <ul>
 *   <li>固定延时级别：通过 {@link DelayLevel} 指定 18 级固定延时</li>
 *   <li>任意延时：通过 {@code delayTimeMillis} 指定精确延时时间</li>
 * </ul>
 *
...
public class DelayMessageProducer {
    ...
    public SendResult sendFixedDelayMessage(String orderId, String content, DelayLevel delayLevel) {
        log.info("Sending fixed delay message: orderId={}, delayLevel={}, content={}",
                orderId, delayLevel, content);

        Message<String> message = MessageBuilder.<String>withTopic("delay-order-topic")
                .tag("delay")
                .keys(orderId)
                .body(content)
        ...
        log.info("Fixed delay message sent: orderId={}, msgId={}, delayLevel={}",
                orderId, result.getMessageId(), delayLevel);
        return result;
    }
    ...
    public SendResult sendCustomDelayMessage(String orderId, String content, long delayMillis) {
        log.info("Sending custom delay message: orderId={}, delayMillis={}, content={}",
                orderId, delayMillis, content);
        ...
        SendResult result = template.syncSend(message);
        log.info("Custom delay message sent: orderId={}, msgId={}, delayMillis={}",
                orderId, result.getMessageId(), delayMillis);
        return result;
    }
