 ...
 *
 * <ul>
 *   <li>同步发送（基础 API + MessageBuilder）
 *   <li>异步发送（CompletableFuture + SendCallback）
 *   <li>单向发送（fire-and-forget）
 *   <li>批量发送（BatchMessage）
 *   <li>MessageMetadataBuilder 模式（统一封装附加参数）
 * </ul>
 *
...
public class OrderProducer {
    ...
    public CompletableFuture<SendResult> createOrderAsync(String orderId, String content) {
        ...
        CompletableFuture<SendResult> future =
                service.asyncSend("order-topic", content, "async", orderId);

        future.thenAccept(
                        result ->
                                log.info(
                                        "Async send completed: orderId={}, msgId={}",
                                        orderId,
                                        result.getMessageId()))
                .exceptionally(
                        ex -> {
                            log.error(
                                    "Async send failed: orderId={}, error={}",
                                    orderId,
                                    ex.getMessage());
                            return null;
                        });
     ...
     * @return 异步发送结果
     */
    public CompletableFuture<SendResult> createOrderAsyncWithTimeout(
            String orderId, String content, long timeoutMillis) {
        log.info(
        ...
        SendCallback callback =
                new SendCallback() {
                    ...
                    public void onSuccess(SendResult result) {
                        log.info(
                                "Callback send success: orderId={}, msgId={}",
                                orderId,
                                result.getMessageId());
                    }
                    ...
                    public void onException(Throwable ex) {
                        log.error(
                                "Callback send failed: orderId={}, error={}",
                                orderId,
                                ex.getMessage(),
                                ex);
                    }
    ...
    public List<SendResult> createOrdersBatch(List<String> orderIds, List<String> contents) {
        ...
        log.info(
                "Batch messages sent: successCount={}",
     ...
     * @return 发送结果
     */
    public SendResult createOrderWithTimeoutAndRetry(
            String orderId, String content, long timeoutMillis, int retryTimes) {
        log.info(
                "Producing order message with timeout={}ms and retry={}: orderId={}",
                timeoutMillis,
                retryTimes,
                orderId);

        MessageMetadataBuilder metadata =
                MessageMetadataBuilder.create().tag("timeout-retry").keys(orderId);

        SendResult result =
                service.send("order-topic", content, metadata, timeoutMillis, retryTimes);
        log.info(
