 ...
 * <p>典型应用场景：
 * <ul>
 *   <li>本地事务执行时间过长，先返回 UNKNOW</li>
 *   <li>网络抖动导致半消息 COMMIT/ROLLBACK 通知丢失</li>
 *   <li>服务宕机恢复后，回查未确认的事务消息</li>
 * </ul>
 *
...
public class OrderTransactionChecker implements TransactionChecker<String> {
     ...
     * 回查本地事务的最终状态。
     *
     * <p>通过 {@code context.getTransactionId()} 查询本地事务日志表，
     * 判断事务是否已提交、已回滚或仍处于未知状态。
     *
     * @param message 半消息载体，包含原始业务数据
     * @param context 事务上下文，提供 transactionId、transactionGroup 等信息
     * @return 本地事务状态：
     *         <ul>
     *           <li>{@link LocalTransactionState#COMMIT_MESSAGE} - 本地事务已提交，提交半消息</li>
     *           <li>{@link LocalTransactionState#ROLLBACK_MESSAGE} - 本地事务已回滚，删除半消息</li>
     *           <li>{@link LocalTransactionState#UNKNOW} - 本地事务状态仍未知，等待下次回查</li>
     *         </ul>
     * @throws Exception 业务异常，框架将其视为 UNKNOW，等待下次回查
     */
    @Override
    public LocalTransactionState check(Message<String> message, TransactionContext context) throws Exception {
        log.info("Checking transaction: txId={}, transactionGroup={}, body={}",
                context.getTransactionId(), context.getTransactionGroup(), message.getBody());
        ...
        try {
            ...
            log.info("Transaction check result: txId={}, state={}",
                    context.getTransactionId(), state);
            return state;
        } catch (Exception e) {
            log.error("Transaction check failed: txId={}, error={}",
