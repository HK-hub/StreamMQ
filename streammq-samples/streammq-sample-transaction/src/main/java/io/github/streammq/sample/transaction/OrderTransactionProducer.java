 ...
 * 订单事务消息生产者示例。
 *
 * <p>演示事务消息发送流程：通过 {@link StreamMessageTemplate#executeInTransaction}
 * 发送半消息并执行本地事务，根据本地事务执行结果决定 COMMIT 或 ROLLBACK。
 * 当本地事务返回 {@link LocalTransactionState#UNKNOW} 时，框架会触发
 * {@link TransactionChecker} 进行回查。
 *
...
public class OrderTransactionProducer {
     ...
     * <p>事务流程：
     * <ol>
     *   <li>发送半消息到 Broker（消费者暂时不可见）</li>
     *   <li>执行本地事务（例如：写数据库）</li>
     *   <li>本地事务成功则 COMMIT，失败则 ROLLBACK，未知则等待回查</li>
     * </ol>
     *
    ...
    public SendResult sendOrderTransaction(String orderContent) {
        ...
        TransactionCallback<String> callback = new TransactionCallback<String>() {
            @Override
            public LocalTransactionState execute(Message<String> message, TransactionContext ctx) {
                log.info("Executing local transaction: txId={}, transactionGroup={}, body={}",
                        ctx.getTransactionId(), ctx.getTransactionGroup(), message.getBody());

                try {
                    // 模拟执行本地事务（例如：写入订单数据库）
                    executeLocalTransaction(message.getBody(), ctx.getTransactionId());

                    log.info("Local transaction committed: txId={}", ctx.getTransactionId());
                    return LocalTransactionState.COMMIT_MESSAGE;
                } catch (Exception e) {
                    log.error("Local transaction failed, rolling back: txId={}, error={}",
        ...
        log.info("Transaction message sent: msgId={}, status={}",
                result.getMessageId(), result.getSendStatus());
        return result;
    }
     ...
     *
     * <p>与上述方法功能相同，但通过 {@code StreamMessageService.sendTransaction} 简化。
     * 适用于不需要直接操作 {@link StreamMessageTemplate} 的场景。
     *
    ...
    public SendResult sendOrderTransactionSimple(String orderContent) {
        ...
        TransactionCallback<String> callback = (message, ctx) -> {
            log.info("Executing local transaction (simple): txId={}", ctx.getTransactionId());
            executeLocalTransaction(message.getBody(), ctx.getTransactionId());
            return LocalTransactionState.COMMIT_MESSAGE;
        };
