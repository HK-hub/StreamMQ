 ...
 * <p>默认实现：
 * <ul>
 *   <li>{@code FixedArrayRetryPolicy} - 对齐 RocketMQ 16 级固定数组
 *       {@code [10s,30s,1m,2m,3m,4m,5m,6m,7m,8m,9m,10m,20m,30m,1h,2h]}</li>
 *   <li>{@code ExponentialBackoffRetryPolicy} - 指数退避（initial=1s, multiplier=2.0, max=2h）</li>
 * </ul>
 *
 * <p>用户可自定义实现并通过配置注入。
 *
...
public interface RetryPolicy {
    ...
    /**
     * 是否应停止重试（消息将进入 DLQ）。
     * 默认实现：{@code reconsumeTimes >= maxReconsumeTimes}，用户可自定义（如根据消息类型/属性决策）。
     *
