 ...
 * 手动 ACK 消费回调接口（并发消费）。
 *
 * <p>使用此接口的 Listener 需在 {@code @StreamMqListener} 中显式设置 {@code acknowledgeMode = MANUAL}，
 * 并通过 {@link ConsumerContext#acknowledge()} 显式 ACK：
 * <ul>
...
public interface StreamMqAckListener<T> {
     ...
     */
    void onMessage(Message<T> message, ConsumerContext context) throws Exception;
}
