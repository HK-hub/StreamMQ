 ...
 *
 * <p>子类只需实现 {@link #doDecide(Message, DlqFailureContext)}，
 * 本类在调用前后统一处理日志记录与告警判断。
 *
...
public abstract class AbstractDlqFailureStrategy implements DlqFailureStrategy {
    ...
    public final DlqFailureDecision decide(Message<?> message, DlqFailureContext context) {
        ...
        if (shouldAlert(context)) {
            log.error("DLQ alert threshold reached: topic={}, dlqAttempts={}, decision={}",
                context.originalTopic(), context.dlqAttempts(), decision.type());
        }
    ...
    /**
     * 判断是否应触发告警。
     * 默认：dlqAttempts + 1 >= alertThreshold 时触发。
     */
