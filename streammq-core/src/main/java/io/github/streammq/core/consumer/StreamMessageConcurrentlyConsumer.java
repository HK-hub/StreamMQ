 ...
 * Consumer 返回 {@link ConsumeAction} 控制后续流程（唯一消费结果表达方式）：
 * <ul>
 *   <li>{@link ConsumeAction#SUCCESS} - 自动 ACK</li>
 *   <li>{@link ConsumeAction#RECONSUME_LATER} - 按 {@code RetryPolicy} 重试</li>
 *   <li>{@code ConsumeAction.defer(Duration)} - 按指定延迟重试</li>
 * </ul>
 *
 * <p>抛出 {@link RuntimeException} 等价于返回 {@link ConsumeAction#RECONSUME_LATER}。
 * 框架不提供手动 ACK/nack/defer 调用，避免双模式冲突。
 *
 * <p>命名说明：实现 {@code onMessage} 的类是"消费者"（Consumer），负责业务处理；
 * 而 {@link StreamMQListener} 是底层"监听器"，负责从 Redis Stream 拉取消息后交给本接口处理。
 *
 * @param <T> body 类型
...
public interface StreamMessageConcurrentlyConsumer<T> extends StreamMessageConsumer<T> {
     ...
     * @return 处理结果动作（{@link ConsumeAction#SUCCESS} / {@link ConsumeAction#RECONSUME_LATER} /
     *         {@code ConsumeAction.defer(Duration)}），返回 null 视为 RECONSUME_LATER
