 ...
 *
 * <p>与 {@link StreamMQConsumer} 完全独立，DLQ 消费者不使用 {@code dlqMode} 混用模式。
 * 启动时框架会严格校验：标注本注解的类必须实现 {@link DlqMessageConsumer}，否则启动失败。
 *
 * <p>DLQ 消费失败由 {@link #failureStrategy()} 决策：
 * <ul>
 *   <li><b>drop</b>（默认）：ACK 消息并丢弃，记录 ERROR 日志</li>
 *   <li><b>retry</b>：ACK 当前消息，重新写入 DLQ Stream（有限次重试后 drop）</li>
 *   <li><b>secondaryDlq</b>：ACK 当前消息，转投到二级 DLQ Stream</li>
 * </ul>
