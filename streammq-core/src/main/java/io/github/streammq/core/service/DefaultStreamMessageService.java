...
public class DefaultStreamMessageService implements StreamMessageService {
    ...
    public <T> SendResult send(String topic, T body, long timeoutMillis) {
        return template.syncSend(
                MessageBuilder.<T>withTopic(topic).body(body).build(), timeoutMillis);
    }
    ...
    @Override
    public <T> SendResult send(
            String topic, T body, String tag, long timeoutMillis, int retryTimes) {
        return template.syncSend(
                MessageBuilder.<T>withTopic(topic).body(body).tag(tag).build(),
                timeoutMillis,
                retryTimes);
    }
                        ...
                        .map(
                                body -> {
                                    MessageBuilder<T> builder =
                                            MessageBuilder.<T>withTopic(topic).body(body);
                                    if (Objects.nonNull(metadata)) {
                                        metadata.applyTo(builder);
                                    }
