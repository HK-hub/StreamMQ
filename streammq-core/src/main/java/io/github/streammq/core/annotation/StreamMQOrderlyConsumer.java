...
/**
 * StreamMQ 顺序消费专用消费者注解。
 *
 * <p>语义上等价于 {@code @StreamMqConsumer(messageModel = MessageModel.ORDERLY)}，
 * 但强制 {@code messageModel = ORDERLY} 且仅允许标注在 {@code StreamMqOrderlyConsumer} 实现类上。
 *
 * <p>使用示例：
 * <pre>{@code
 * @Component
 * @StreamMqOrderlyConsumer(topic = "order-topic", consumerGroup = "order-consumer-group")
 * public class OrderOrderlyConsumer implements StreamMqOrderlyConsumer<Order> {
 *     @Override
...
@Documented
public @interface StreamMQOrderlyConsumer {
