 ...
 * <ul>
 *   <li>{@link ConsumeAction#SUCCESS} - 消费成功，自动 ACK，下一条继续</li>
 *   <li>{@link ConsumeAction#RECONSUME_LATER} - 暂停当前 shard 一小段时间（默认 10ms），
 *       消息留在 PEL 等待重新消费同一消息（与并发消费的 RECONSUME_LATER 行为不同，
 *       顺序消费不会将消息重新入队重试，而是暂停 shard 后原地重试）</li>
 * </ul>
 *
