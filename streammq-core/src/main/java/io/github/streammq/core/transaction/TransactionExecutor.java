 ...
 * <p><b>简化模式 vs 完整模式：</b>
 * <ul>
 *   <li><b>简化模式</b>（本接口）：在当前线程同步执行本地事务，无需配置 {@code TransactionScanner}。
 *       适用于单实例部署或对事务一致性要求不高的场景。
 *       风险：如果 JVM 在事务执行过程中崩溃，半消息可能永远不会被提交或回滚。</li>
 *   <li><b>完整模式</b>：通过 {@code TransactionScanner} 定期扫描半消息并触发 {@code TransactionChecker} 回查。
 *       适用于多实例部署或需要高可靠事务保证的场景。
 *       需要配置 {@code streammq.transaction.enabled=true} 并实现 {@code TransactionChecker}。</li>
 * </ul>
 *
 * <p><b>可靠性保证：</b>
 * <ul>
 *   <li>简化模式：消息持久化到 Redis（取决于 Redis AOF 配置），但本地事务与消息发送不是原子操作</li>
 *   <li>完整模式：通过回查机制保证最终一致性，即使 JVM 崩溃也能通过回查恢复</li>
 * </ul>
 *
