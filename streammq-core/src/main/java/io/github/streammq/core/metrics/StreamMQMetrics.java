 ...
 * StreamMQ 指标收集接口。
 *
 * <p>定义了 StreamMQ 各核心环节的指标埋点契约，由具体的实现（如基于 Micrometer 的
 * {@code MicrometerStreamMQMetrics}）提供实际的指标上报能力。
 *
 * <p>实现类应保证线程安全，并允许在指标注册表不可用时以 no-op 方式安全降级，
 * 即所有 {@code record*} 方法都不得抛出异常影响业务主流程。
 *
 * <p>指标覆盖范围：
 * <ul>
 *   <li>发送：{@link #recordSend}</li>
 *   <li>消费：{@link #recordConsume}</li>
 *   <li>重试：{@link #recordRetry}</li>
 *   <li>死信：{@link #recordDlq}</li>
 *   <li>延时投递：{@link #recordDelayDelivery}</li>
 *   <li>事务提交 / 回滚 / 回查：{@link #recordTransactionCommit} /
 *       {@link #recordTransactionRollback} / {@link #recordTransactionCheck}</li>
 * </ul>
 *
