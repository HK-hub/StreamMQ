 ...
 *
 * <p>Listener 在 {@code onMessage} 中返回本类实例控制后续流程，框架以返回值为唯一标准，
 * 不再支持手动 {@code context.acknowledge()/nack()/defer()} 调用，避免双模式冲突：
 * <ul>
 *   <li>{@link #SUCCESS} - 消费成功，自动 ACK，从 PEL 移除</li>
 *   <li>{@link #RECONSUME_LATER} - 消费失败，按 {@code RetryPolicy} 计算延迟后写入 retry ZSet 重投</li>
 *   <li>{@link #defer(Duration)} - 消费失败，使用业务指定的延迟写入 retry ZSet 重投（覆盖 RetryPolicy）</li>
 * </ul>
 *
 ...
 * }</pre>
 *
 * <p>线程安全：{@link #SUCCESS} 与 {@link #RECONSUME_LATER} 为不可变单例常量，可安全共享；
 * {@link #defer(Duration)} 每次返回新实例，仅供单条消息消费使用。
 *
...
public final class ConsumeAction {
    ...
    public static final ConsumeAction RECONSUME_LATER = new ConsumeAction(Type.RECONSUME_LATER, null);
