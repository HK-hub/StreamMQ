 ...
 *
 * <p>消息在发送时会将 Codec 名称写入 {@code "compressed"} 字段（如 {@code "gzip"}），
 * 消费端通过此注册表按名称查找对应的 Codec 实例进行解压。
 *
 * <p>内置 Codec 在自动装配时注册；用户自定义 Codec 可通过实现
 * {@link CompressionCodec} 并注册 Spring Bean 的方式添加。
 *
