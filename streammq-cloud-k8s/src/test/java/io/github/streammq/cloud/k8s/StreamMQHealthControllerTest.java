 ...
 *
 * <p>使用 {@link WebMvcTest} 加载 Web 层，通过 {@link MockBean} 注入模拟的
 * {@link StreamMQListenerContainer}，使用 MockMvc 验证 JSON 响应。
 * 容器不存在时的降级场景使用独立 MockMvc 测试。
 */
...
class StreamMQHealthControllerTest {
    ...
    void liveness_returnsUpStatus() throws Exception {
        mockMvc.perform(get("/streammq/health/liveness"))
    ...
    void readiness_returnsCorrectConsumerCount() throws Exception {
        when(container.isRunning()).thenReturn(true);
        when(container.getConsumers()).thenReturn(List.of(
            new StreamMQListenerContainer.ConsumerMetadata("order-topic", "order-group",
                Object.class, Object.class),
            new StreamMQListenerContainer.ConsumerMetadata("payment-topic", "payment-group",
                Object.class, Object.class)
        ));
        mockMvc.perform(get("/streammq/health/readiness"))
            .andExpect(status().isOk())
    ...
    void readiness_whenContainerNotAvailable_returnsDegraded() throws Exception {
        ...
        standaloneMockMvc.perform(get("/streammq/health/readiness"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.ready").value(false))
            .andExpect(jsonPath("$.consumerCount").value(0));
    }
