...
public class StreamConsumerBenchmark {
  ...
  public void setup() {
    ...
    DefaultMessageConverter converter = new DefaultMessageConverter(new JacksonJsonSerializer<>());
    RedissonStreamProducerFactory producerFactory =
        new RedissonStreamProducerFactory(redisson, converter);
    template = new DefaultStreamMessageTemplate(producerFactory, "benchmark-producer", converter);
    ...
    try {
      stream.createGroup(
          StreamCreateGroupArgs.name(CONSUMER_GROUP).makeStream().id(new StreamMessageId(0, 0)));
  ...
  public void consumeThroughput() throws Exception {
    ...
    var messages =
        stream.readGroup(
            CONSUMER_GROUP, CONSUMER_NAME, StreamReadGroupArgs.neverDelivered().count(BATCH_SIZE));
    ...
    if (messages != null && !messages.isEmpty()) {
      ...
      for (var entry : messages.entrySet()) {
        Message<String> msg =
            MessageBuilder.<String>withTopic(TOPIC).tag("consumer-test").body(payload).build();
