 ...
 * <p>工作流程：
 * <ul>
 *   <li>{@link #beforeSend(Message)}：调用 {@link StreamMQTracing#injectProducerSpan(Message)}
 *       创建生产者 Span 并将 W3C {@code traceparent} 注入消息属性</li>
 *   <li>{@link #afterSend(Message, SendResult)}：根据发送结果结束 Span（成功 / 失败 + 错误信息）</li>
 *   <li>{@link #onException(Message, Exception, InvokeTiming)}：异常时以失败状态结束 Span</li>
 * </ul>
 *
