...
class StreamMQTopologyServiceTest {
    ...
    void getMessageTrace_shouldBuildCompleteTrace() {
        TraceRecord send = new TraceRecord("m1", "order-topic", "producer-group",
            TraceType.SEND, true, 1000L, 5L, "trace-1", Map.of());
        TraceRecord consume = new TraceRecord("m1", "order-topic", "order-group",
            TraceType.CONSUME, true, 1500L, 50L, "trace-1", Map.of());
        ...
        assertThat(trace.routePath()).containsExactly(
            "Producer", "Topic:order-topic", "Group:order-group", "Consumer:order-group");
    ...
    void getMessageTrace_failedTrace_shouldBeFailed() {
        TraceRecord send = new TraceRecord("m2", "order-topic", "producer-group",
            TraceType.SEND, true, 1000L, 5L, "trace-2", Map.of());
        TraceRecord consume = new TraceRecord("m2", "order-topic", "order-group",
            TraceType.CONSUME, false, 1500L, 50L, "trace-2", Map.of());
    ...
    void getTopicTraces_shouldGroupByMessageId() {
        TraceRecord r1 = new TraceRecord("m1", "order-topic", "g1",
            TraceType.SEND, true, 1000L, 1L, "t1", Map.of());
        TraceRecord r2 = new TraceRecord("m2", "order-topic", "g2",
            TraceType.SEND, true, 2000L, 1L, "t2", Map.of());
        ...
        assertThat(traces).extracting(MessageTrace::messageId).containsExactlyInAnyOrder("m1", "m2");
    }

    @Test
    @DisplayName("getTopicTopology 应构建生产者、消费者与路由")
    void getTopicTopology_shouldBuildGraph() {
        TraceRecord send1 = new TraceRecord("m1", "order-topic", "producer-group",
            TraceType.SEND, true, 1000L, 5L, "t1", Map.of());
        TraceRecord send2 = new TraceRecord("m2", "order-topic", "producer-group",
            TraceType.SEND, true, 2000L, 5L, "t2", Map.of());
        when(traceService.queryByTopic(eq("order-topic"), anyLong(), anyLong()))
            .thenReturn(List.of(send1, send2));
        ConsumerMetadata metadata = new ConsumerMetadata("order-topic", "order-group",
