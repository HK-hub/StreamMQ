 ...
 * <ul>
 *   <li>慢消费诊断：基于追踪数据识别消费耗时异常</li>
 *   <li>积压诊断：基于追踪数据分析积压量</li>
 *   <li>死信队列诊断：识别失败原因与主题分布</li>
 *   <li>消息画像：聚合发送与消费追踪记录</li>
 * </ul>
 *
...
class StreamMQDiagnosticsMockTest {
    ...
    class SlowConsumeDiagnostics {
        ...
        void shouldIdentifySlowConsume() {
            ...
            TraceRecord slowRecord = new TraceRecord(
                "msg-001", TOPIC, GROUP, TraceType.CONSUME,
                true, now, 6000L, "trace-001", Map.of()
            );
    ...
    class MessageProfileTests {
        ...
        void shouldReturnNullWhenNoTrace() {
            when(traceService.queryByMessageId(anyString()))
                .thenReturn(List.of());
        ...
        void shouldBuildProfileWithTraceData() {
            ...
            TraceRecord sendRecord = new TraceRecord(
                "msg-001", TOPIC, "producer-group", TraceType.SEND,
                true, now - 1000L, 10L, "trace-001",
                Map.of("tag", "order", "keys", "key-1")
            );
            TraceRecord consumeRecord = new TraceRecord(
                "msg-001", TOPIC, GROUP, TraceType.CONSUME,
                true, now, 5L, "trace-001",
