 ...
 * <p>装配条件：
 * <ul>
 *   <li>classpath 中存在 {@link StreamMessageTemplate}（表示 StreamMQ 已被启用）</li>
 *   <li>{@code spring.cloud.stream.streammq.binder.enabled=true}（默认 true）</li>
 * </ul>
 *
 * <p>装配内容：
 * <ol>
...
public class StreamMQBinderConfiguration {
     ...
     *
     * <p>注册为 {@link org.springframework.cloud.stream.binder.Binder} 类型的 Bean，
     * Spring Cloud Stream 会自动发现并使用。
     * 当应用中存在多个 Binder 时，通过 {@code spring.cloud.stream.default-binder=streammq} 指定。
     *
