...
/**
 * StreamMQ 消费者扩展属性（Plain POJO，不继承 {@link
 * org.springframework.cloud.stream.binder.ConsumerProperties}）。
 *
 * <p>本类仅承载 StreamMQ 特有的消费端配置项，由 Spring Cloud Stream 的 {@link
 * org.springframework.cloud.stream.binder.ExtendedConsumerProperties} 包装后 传递给 {@link
 * StreamMQMessageBinder}。
 *
 * <p>用户可通过 {@code spring.cloud.stream.streammq.bindings.<bindingName>.consumer.*} 前缀进行配置， 或通过
 * {@code spring.cloud.stream.streammq.default.consumer.*} 配置全局默认值。
 *
 * <p>属性说明：
 *
 * <ul>
 *   <li>{@code selectorExpression} - Tag 过滤表达式，默认 "*" 表示全部接收
 *   <li>{@code selectorType} - 过滤类型，默认 "TAG"
 *   <li>{@code shardCount} - 顺序消费分区数，默认 4
 *   <li>{@code enableMsgTrace} - 是否启用消息追踪，默认 false
 *   <li>{@code concurrency} - 消费线程数（覆盖 Binder 全局默认），默认 -1 表示使用全局值
 *   <li>{@code maxAttempts} - 最大重试次数（覆盖 Binder 全局默认），默认 -1 表示使用全局值
 * </ul>
 *
