 ...
 * <p>核心职责：
 * <ol>
 *   <li>继承 {@link MessageProducerSupport}，作为 Spring Integration 消息源</li>
 *   <li>实现 {@link StreamMessageConcurrentlyConsumer}，接收 StreamMQ 消息</li>
 *   <li>将 StreamMQ {@link Message} 转换为 Spring Integration 消息并通过 {@link #sendMessage} 输出</li>
 *   <li>在 {@link #doStart()} 时向 {@link StreamMQListenerContainer} 注册消费者</li>
 *   <li>在 {@link #doStop()} 时由容器停止消费</li>
 * </ol>
 *
 * <p>消费结果映射：本生产者始终返回 {@link ConsumeAction#SUCCESS}，
 * 消费失败的重试由 Spring Cloud Stream 的错误处理机制（errorChannel / DLQ）负责。
 *
...
public class StreamMQMessageProducer extends MessageProducerSupport
    ...
    @Override
    public ConsumeAction onMessage(Message<Object> message, ConsumeContext context) throws Exception {
    ...
    private org.springframework.messaging.Message<Object> convert(Message<Object> streamMessage) {
        ...
        if (Objects.nonNull(streamMessage.getMessageId())) {
            builder.setHeader(StreamMQMessageHandler.HEADER_MESSAGE_ID, streamMessage.getMessageId().toString());
        }
        ...
        if (Objects.nonNull(streamMessage.getShardingKey())) {
            builder.setHeader(StreamMQMessageHandler.HEADER_SHARDING_KEY, streamMessage.getShardingKey());
        }
        builder.setHeader(StreamMQMessageHandler.HEADER_RECONSUME_TIMES, streamMessage.getReconsumeTimes());
        builder.setHeader(StreamMQMessageHandler.HEADER_BORN_TIMESTAMP, streamMessage.getBornTimestamp());
    ...
    protected void doStart() {
        ...
        log.info("StreamMQMessageProducer 已启动: topic={}, group={}, selectorExpression={}, shardCount={}",
            topic, group, consumerProperties.getSelectorExpression(), consumerProperties.getShardCount());
    }
     ...
     *
     * <p>由于 {@link StreamMQConsumer} 是注解类型，无法直接实例化，
     * 此处使用 JDK 动态代理创建一个代理实例，所有方法返回值由 {@link StreamMQConsumerProperties} 提供。
     *
     * @return 注解代理实例
     */
