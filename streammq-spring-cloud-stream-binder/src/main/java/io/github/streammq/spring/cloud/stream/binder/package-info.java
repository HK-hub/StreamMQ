 ...
 * <h2>依赖说明</h2>
 *
 * <p>本模块直接依赖 {@code streammq-redisson}（非仅 {@code streammq-spring-boot-starter}）， 因此使用本 Binder
 * 的用户会传递性引入 Redisson 作为 Redis 客户端。如果你的应用已经使用 Lettuce（Spring Boot 3 默认）， 需要明确接受两个 Redis 客户端共存——
 * 二者连接到同一 Redis 实例是安全的，但会增加少量 jar 体积。
 *
 * <h2>当前限制</h2>
 *
 * <ul>
 ...
 *       计划支持。
 *   <li><b>无独立 Broker 协议</b>：复用 StreamMQ 的 Redisson 适配层，不暴露 Spring Cloud Stream 抽象下的独立
 *       connector——这与 Kafka/RabbitMQ Binder 不同。
