...
public class StreamMQMessageHandler implements MessageHandler {
  ...
  public void handleMessage(Message<?> message) throws MessagingException {
    ...
    try {
      ...
      if (Objects.nonNull(result) && result.getSendStatus() == SendStatus.SEND_OK) {
        ...
        String errorMsg =
            Objects.nonNull(result) ? result.getSendStatus().name() : "SendResult is null";
        log.warn("消息发送失败: topic={}, status={}", topic, errorMsg);
        sendError(
            message,
            new MessagingException(
                message, new RuntimeException("StreamMQ send failed: " + errorMsg)));
      }
   ...
   *
   * <p>注意：不应在此处将 {@code String} 转换为 {@code byte[]}，否则 {@code DefaultMessageConverter} 会用 Jackson 将
   * {@code byte[]} 序列化为 Base64 字符串，消费端反序列化得到的是 Base64 字符串而非原始文本。
   *
