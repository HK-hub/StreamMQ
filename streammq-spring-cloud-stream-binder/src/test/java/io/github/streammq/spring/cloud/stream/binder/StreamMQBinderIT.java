 ...
 *
 * <p>使用本地 Redis（127.0.0.1:6379）进行真实的消息收发，
 * 验证通过标准 Spring Cloud Stream API（StreamBridge + Consumer）与 StreamMQ Binder 的完整链路。
 *
 * <p>测试场景：
 * <ul>
 *   <li>通过 {@link StreamBridge} 发送消息到 Spring Cloud Stream binding</li>
 *   <li>StreamMQ Binder 将消息写入 Redis Stream</li>
 *   <li>StreamMQ Consumer 从 Redis Stream 消费消息</li>
 *   <li>Binder 将消息转换为 Spring Messaging 消息并传递给 {@code Consumer<String>} Bean</li>
 * </ul>
 *
 * @author StreamMQ Contributors
 * @since 0.1.0
 */
@SpringBootTest(classes = StreamMQBinderIT.TestApplication.class,
        properties = {
...
class StreamMQBinderIT {
     ...
     *
     * <p><b>不使用 {@code flushdb()} 清空 Redis</b>：监听器容器在 Spring 上下文启动时即创建消费组，
     * 此时调用 {@code flushdb()} 会删除消费组但 {@code RedissonStreamListener.groupCreated} 标志仍为 true，
     * 导致后续 {@code readGroup} 持续失败。消费组通过 {@code XREADGROUP >} 只投递从未投递过的新消息，
     * 已消费并 ack 的消息不会被重复投递，因此仅清空收集器即可保证测试隔离。
     */
    @BeforeEach
    void clearState() {
        collector.clear();
    ...
    void shouldSendAndReceiveMultipleMessages() {
        ...
        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(collector.getReceived()).contains("msg-0", "msg-1", "msg-2", "msg-3", "msg-4"));
    }
     ...
     * 通过 {@code spring.cloud.stream.source.sendMsg} 声明输出 binding 名 {@code sendMsg-out-0}，
     * 由 StreamMQ Binder 创建对应的生产者。
     */
