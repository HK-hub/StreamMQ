 ...
 * <ul>
 *   <li>通过 {@link StreamBridge} 发送带 headers（tag、keys）的消息</li>
 *   <li>发送用户自定义 properties，验证透传正确性</li>
 *   <li>多种消息类型（String、JSON payload）</li>
 *   <li>验证 {@link StreamMQMessageHandler} 常量正确使用</li>
 *   <li>多条消息批量发送与接收</li>
 * </ul>
 *
 ...
 * @since 0.1.0
 */
@SpringBootTest(classes = StreamMQBinderExtendedIT.TestApplication.class,
        properties = {
...
class StreamMQBinderExtendedIT {
    ...
    @Autowired
    private TestMessageCollector collector;
    ...
    void shouldSendWithHeadersAndReceiveThem() {
        streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload("hello-with-headers")
    ...
    void shouldPropagateUserDefinedProperties() {
        streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload("user-props-test")
    ...
    void shouldSendAndReceiveStringMessage() {
        streamBridge.send("sendMsg-out-0",
                MessageBuilder.withPayload("plain-text-message").build());
    ...
    void shouldSendAndReceiveJsonPayload() {
        ...
        streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload(jsonPayload)
                .setHeader("contentType", "application/json")
                .build());

        await().atMost(10, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(collector.getPayloads()).contains(jsonPayload));
    }
    ...
    void shouldSendMultipleMessages() {
        for (int i = 0; i < 5; i++) {
            streamBridge.send("sendMsg-out-0",
                    MessageBuilder.withPayload("batch-msg-" + i).build());
        }

        await().atMost(15, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(collector.getPayloads())
                        .contains("batch-msg-0", "batch-msg-1", "batch-msg-2",
    ...
    void shouldUseStreamMQMessageHandlerConstants() {
        streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload("constant-test")
                .setHeader(StreamMQMessageHandler.HEADER_TAG, "priority-high")
                .setHeader(StreamMQMessageHandler.HEADER_KEYS, "order-456")
                .build());

        await().atMost(10, TimeUnit.SECONDS).untilAsserted(() ->
                assertThat(collector.getPayloads()).contains("constant-test"));
    }
     ...
     * 测试 Spring Boot 应用配置。
     *
     * <p>定义一个函数式 Consumer {@code receiveMsg} 作为消息接收端，
     * Spring Cloud Stream 会自动创建 {@code receiveMsg-in-0} 绑定。
     * 通过 {@code spring.cloud.stream.source.sendMsg} 声明输出 binding 名
     * {@code sendMsg-out-0}，由 StreamMQ Binder 创建对应的生产者。
     */
