...
class StreamMQBinderExtendedIT {
  ...
  void shouldSendWithHeadersAndReceiveThem() {
    ...
    await()
        ...
        .untilAsserted(() -> assertThat(collector.getPayloads()).contains("hello-with-headers"));
  }
  ...
  void shouldPropagateUserDefinedProperties() {
    ...
    await()
        .atMost(10, TimeUnit.SECONDS)
        .untilAsserted(() -> assertThat(collector.getPayloads()).contains("user-props-test"));
  }
  ...
  void shouldSendAndReceiveStringMessage() {
    streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload("plain-text-message").build());

    await()
        .atMost(10, TimeUnit.SECONDS)
        .untilAsserted(() -> assertThat(collector.getPayloads()).contains("plain-text-message"));
  }
  ...
  void shouldSendMultipleMessages() {
    for (int i = 0; i < 5; i++) {
      streamBridge.send("sendMsg-out-0", MessageBuilder.withPayload("batch-msg-" + i).build());
    }
        ...
        .untilAsserted(
            () ->
                assertThat(collector.getPayloads())
                    .contains(
                        "batch-msg-0", "batch-msg-1", "batch-msg-2", "batch-msg-3", "batch-msg-4"));
  }
